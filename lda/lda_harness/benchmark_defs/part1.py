# -*- coding: utf-8 -*-
"""BENCHMARK_DEFS 数据分片 1/5（F-08 巨石治理 · v0.9.117）。

覆盖 key：B1 … B76（共 94 条，按原 `benchmarks.py` 文件顺序**连续**切分）。
...
from ..golden import (
    b10_gate_fidelity, b11_ring_spectrum_match, b12_resonator_frequency, b13_coupler_coupling,
    b14_dc_coupling_length, b15_bragg_wavelength, b16_mmi_length, b17_jj_critical_current,
    b18_purcell_factor, b19_link_passivity_bound, b1_mie_qscat, b20_mzi_fsr, b21_phc_resonance,
    b22_qres_frequency, b23_fluxonium_lc_limit, b24_tcoup_geff, b25_tunable_transmon_f01,
    b26_dispersive_shift, b27_cz_gate_time, b2_soi_waveguide_neff, b3_fp_fsr_nm, b4_ring_fsr_nm,
    b5_ybranch_split_loss_dB, b6_grating_coupling_eff, b7_crossing_crosstalk_dB,
    b8_taper_transmission, b9_transmon_frequency, golden_b65, golden_b66, golden_b67,
    golden_b68, golden_b70, golden_b71, golden_b73, golden_b74, golden_b75, golden_b76,
    s10_lvs_multilayer_verdict, s11_large_scale_verdict, s12_array_distribution_verdict,
    s13_design_yield_anchor, s1_power_budget_margin_dB, s2_channel_plan_no_collision,
    s3_osnr_budget, s4_fidelity_budget, s5_worst_case_budget, s6_detector_margin,
    s7_statistical_margin_p5_anchor, s8_statistical_osnr_p5_anchor, s9_lvs_verdict,
)
from ..b28_modulator_vpi_anchor import (
    b28_modulator_vpi,
)
from ..b29_thermal_phase_anchor import (
    b29_thermal_phase_efficiency,
)
from ..b30_readout_anchor import (
    b30_readout_fidelity,
)
from ..b33_detector_bandwidth_anchor import (
    b33_detector_bandwidth,
)
from .._batch_b_numeric import (
    golden_b34, golden_b36, golden_b37, golden_b40, golden_b41,
)
from .._batch_b2_numeric import (
    golden_b42, golden_b43, golden_b44, golden_b45, golden_b46, golden_b47, golden_b48,
    golden_b49, golden_b50, golden_b51,
)
from .._batch_b3_numeric import (
    golden_b52, golden_b53, golden_b54, golden_b55, golden_b56, golden_b57, golden_b58,
    golden_b59, golden_b60, golden_b61, golden_b62, golden_b63, golden_b64,
)
from ..b31_soref_bennett_anchor import (
    b31_soref_bennett_phase_shift,
)
from ..b32_qcse_anchor import (
...
DEFS = {
    ...
    "B3": {
        ...
        "note": ("Airy 解析公式 FSR=λ²/(2nL)。"
                 ...
                 "🔴 口径复核（波次4 §7）：m≈12.9 使一阶连续化偏置达 6.7%（8.1nm，超 tol=1.0）；"
                 "若未来用作强判据（tol 收紧），建议把参数点移到 m≫1（增大 L）以消除该偏置——"
                 "不改变当前按频域周期口径的取值。"),
    },
    "B4": {
    ...
    "B20": {
        ...
        "note": ("MZI 干涉传输 T=½(1+cos(2π·n_eff·ΔL/λ))；FSR=λ²/(n_eff·ΔL)"
                 ...
                 "🔴 口径复核（波次4 §7）：tol=1e-6 与「微分近似 vs 精确定义」的固有差（1e-3）"
                 "相差 1000 倍 ⇒ 该锚恒为降级/自证桩口径，**不能**升 strict；这是口径限制"
                 "非数值缺陷，不属盲区。详见 `LDA_blindspot_supplement_2026-09-21.md` §7。"),
    },
    "B21": {
        ...
        "note": ("2D 光子晶体腔 = 均匀高折射率波导腔（L_cav，n_eff=(n_core+n_clad)/2）"
                 ...
                 "确定性物理定律锚，LLM 不进判决路径。"
                 "🔴 口径复核（波次4 §7）：n_eff 采用算术平均 (n_core+n_clad)/2 作 FP/布拉格带边"
                 "一阶近似；与偏振严格零阶值的差可达 +8.3%/-23.5%，而 tol=66nm(=3%×golden)"
                 "对此近似粗糙度保守归 degraded_ordinal（同 B 路径纪律），不进死标量判决列。"),
    },
    "B22": {
    ...
    "E10": {
        ...
        "note": "实证锚升格（v0.9.76 · U3 唯一可诚实接的环类锚）：此前 E-RING-FSR 仅作"
                ...
                "残差立刻爆到 tol 外）。"
                "②**残差 +0.31nm(+3.6%) 的归因（2026-09-21 实测证伪旧版「弯曲几何」归因）**："
                "实测 golden 的 n_g=4.18 是**真实环器件**群折射率（环形谐振反演）；候选解的是"
                "半矢量**直波导**模型 n_g=4.023（与生产候选逐位吻合）。"
                "🔴 **旧版 note 的「弯曲使 n_g 高 ~0.157」已被本项第一原理实证证伪**：用共形变换"
                "（Heiblum-Harris）算弯曲波导 n_g，R=10µm 时弯曲修正仅 Δn_g≈+0.0038（∝1/R²，"
                "10→20→30µm 各减半/降为 1/9，误差<1%）；要复现 0.157 需 R≈1.55µm"
                "（辐射损耗>10dB/turn，物理不可能）。故 0.157 的差距**不能**归因于弯曲几何。"
                "残差真因 = **参数化模型（半矢量直波导 + Sellmeier 色散）vs 真实环器件**"
                "（工艺粗糙度/材料/几何）的量纲差，非几何不对齐；即便叠加弯曲修正，E10 残差仅"
                "从 0.313→0.304nm（几乎无改善）⇒ **E10 确无内部升格路径**。详见 `LDA_wave7_closeout_plan_2026-09-21.md` §3。"
