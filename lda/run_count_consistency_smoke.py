...
断言维度（全部死标量，LLM 不进判决路径）：
  ...
  5. README 宣传串：动态构造「22 引擎 + 11 包 = 33 类端到端（光子 15 + 量子 7）」
     「42 题（B1-B27 + E1-E7 + S1-S8）」「CI core N 条」断言 README.md 包含；
...
class CountConsistencySmoke(unittest.TestCase):
    ...
    # ---- 3. 题库 ----
    def test_benchmark_order_42(self):
        self.assertEqual(len(self.benchmark_order), 42,
                         f"BENCHMARK_ORDER 应 42 题，实际 {len(self.benchmark_order)}")

    def test_benchmark_b27_e7_s8_split(self):
