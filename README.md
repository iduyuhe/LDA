...
- ⚠️ **判决路径独立性（D-64 如实披露）**：golden 真实只是必要条件，candidate 还必须**独立求解**。当前 7 道实证锚中**仅 E2 接通独立求解器**（FDFD 1.959 ↔ 实测 1.892，|diff|=0.067），**其余 6 道（E1、E3-E7）candidate 仍为占位自证（candidate≡golden，|diff|≡0）** —— 其 PASS 只能算「自洽」，**不算「验证」**，已在 note / 报告 candidate_desc / CI smoke 断言三处如实标注（宁可红，不可假绿）。注意：**E1 的 golden 已升 A 级，但不改变其判决路径仍为自证桩这一事实**（且标量 FDFD 对高对比度 SOI 差 10%，即便接入也必 FAIL，需待 R16 亚网格 ε 平均）。复制 E2 模式到其余六道是下一阶段主线；真实晶圆厂 NDA 实测仍属发动期。
- ⚠️ **候选求解器的精度边界（D-65 如实披露）**：FDFD 本征模求解器在亚微米特征上**网格过粗**——0.3µm 芯厚在 dl=λ/24 下仅约 4.6 格，阶梯边界随计算窗口改变对齐位置，致 n_eff 在窗口间跳变（SOI 侧达 0.25）、n_g 散射 **±0.04~0.08**（对照：厚 SiN 800×800 约 12 格分辨，n_g 散射仅 0.0015）。因此 **E2 只判「量级一致 + 判决路径真实」，不宣称「精度验证」**——其 0.10 容差中大部分是数值不确定度而非物理裕度。判定本身是鲁棒的（5 个计算窗口全部落在容差内，已钉进 smoke），但根治需亚网格 ε 平均 + 更细网格（登记为 **R16**）。
