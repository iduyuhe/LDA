...
> 当前版本：**v0.8.11**（2026-08-26 · 实证锚语料扩充：harness 题库 30→34 题（E1-E7 实证锚，新增 SOI crossing 插入损耗/串扰 + MMI 1×2 过量损耗 + 厚 SiN 传播损耗，全部可溯源真实文献）+ 芯片案例扩展（MZI 干涉网络第三案例 + 链路引擎传播语义修复）+ 基准对照验证闭环报告（15 引擎解析锚 rel + 实证锚 9 条语料全对照）+ 芯片级版图导出（IO 光栅接入 + 版图统计 + 芯片级 DRC）+ loss/效率类引擎（Y-branch/光栅 eff/crossing/MMI EL/SiN PL 五引擎补齐语料缺口）· v0.8.10 首轮持续维护（内核纵深五击 + 芯片级验收闭环 + 器件库主流封口 + 流片级验证管道 + 计数一致性门禁：MZI/PhC 腔/λ/4 读出/Fluxonium/可调耦合器五类内核（B20-B24）+ 可调 transmon/色散读出配对/CZ 门（B25-B27）+ 引擎闭环 **15 引擎 + 11 包 = 26 类端到端**（光子 8 + 量子 7）+ **芯片级四锚验收**（无源界/级联乘法性/能量守恒/完整性）+ 仿真级芯片设计闭环演示（WDM 收发 + 量子读出链路）+ **流片级验证管道**（PDK→DRC→SS/TT/FF 工艺角→实测回流，门3 接口就绪）+ harness 题库 **34 题**（B1-B27 + E1-E7）+ CI core 48 条）
