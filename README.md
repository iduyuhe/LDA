...
> 当前版本：**v0.5**（2026-08-23 · 系统级里程碑：M7 收口（可调 WDM + 量子门/纠错拓扑 + 大规模基准）+ 护城河两件（L0 IR 开放标准 + 验证合约工业化）+ Track A 逆设计纵深（谱形目标 → 形状 → 混合 → 多波长 → 3D adjoint → 3D 截面 → 3D 端口联合验收）+ 四十七面板）
...
**v0.5 的核心能力（D-36~D-82，系统级里程碑）**：给系统一个设计目标（如"f01=5GHz 的 Transmon"、"FSR=17.5nm 的环形分波器"、"N qubit 频率复用读出"、"多环 WDM × 量子读出混合系统"、"8 信道 WDM × 8 qubit 联合压测"），LDA 自动完成 **参数搜索 → 真实求解器双重验证 → 返回被验证过的设计**，并以**统一设计包（DesignPackage）**格式交付——LLM 不进判决路径，是否 PASS 由死标量比对决定。**v0.5 在 v0.4 真实化（真实版图基元 + 2D/3D 端口 S 参数验收 + 伴随法逆设计 + 流水线真实化）之上新增系统级能力**：①**M7 系统级三件**——热光可调 WDM（D-73，物理定律锚 Δλ/λ=(dn/dT)·R_th·P/n_eff）、量子门/纠错拓扑（D-74，门库幺正性+surface code k=1+CR 门）、大规模系统基准（D-75，8 WDM×8 qubit 联合压测 + 容量自洽 + 四边界）；②**护城河两件**——L0 IR 开放标准（D-76，ir_spec + JSON Schema + 零漂移校验）、验证合约工业化（D-77，全量回归统一入口 27 PASS + 性能基准 + 基线漂移监控）；③**Track A 逆设计纵深四阶**——谱形目标（D-80：分束比/模式匹配/多波长谱形 FOM）、形状逆设计（D-81：宽度曲线控制点 + 可制造性内建 + Pareto 多目标）、形状+拓扑混合（D-82：分层表达，混合增益 3.06×）。
