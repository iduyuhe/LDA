...
覆盖：
  ① harness 实证锚题解析（BENCHMARK_DEFS 44 = B1-B27 + E1-E7 + S1-S10；E 题 golden 来自实测语料；
...
def main():
    ...
    check("BENCHMARK_DEFS 44 题（B1-B27+E1-E7+S1-S10）", len(BENCHMARK_DEFS) == 44
    ...
    check("参考候选 44/44 PASS（双 ground）", npass == len(specs) == 44,
          f"{npass}/{len(specs)}")
