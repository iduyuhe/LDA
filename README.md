...
> 当前版本：**v0.6**（2026-08-24 · 3D 逆设计纵深 + QEDA 求解器级补强：破 3D 诚实边界（3D Yee 显式转置伴随 Mᵀ 1e-15 → 3D 截面 → 3D 端口验收 → 谱形×3D → 3D numba 性能 20×+ → **3D voxel 拓扑**）+ transmon-resonator 色散读出三能级严格求解（α 修正必要性 31×）+ QEDA 纵深三件套（多能级展开收敛 / Rabi+AC Stark / 读出串扰 ZZ 耦合）+ 生态共建闭环（harness B1-B18 · 主权依赖 A/B/C · 社区提交入口 · **评审流 + 提案落地**）+ 五十五面板）
