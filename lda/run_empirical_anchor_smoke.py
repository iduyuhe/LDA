...
覆盖：
  ① harness 实证锚题解析（BENCHMARK_DEFS 21 = B18 + E3；E 题 golden 来自实测语料）
  ② 参考候选 21/21 PASS（物理定律 + 实证锚双 ground）
...
def main():
    ...
    check("BENCHMARK_DEFS 21 题（B18+E3）", len(BENCHMARK_DEFS) == 21
    ...
    check("参考候选 21/21 PASS（双 ground）", npass == len(specs) == 21,
          f"{npass}/{len(specs)}")
