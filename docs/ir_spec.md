...
- **机器优先**：一切结构可序列化为纯 dict / JSON，便于 agent 间传递、经 L1 传输、落库 diff；**L0 内部表示不依赖任何商业 EDA 格式；L0 层自身也不 import 任何商业或第三方 GDSII/OASIS 读写库**（此为 **L0 的零依赖原则**，非全仓禁令——版图/导出层如需对接生态格式，按 **B 级「借今踢后」** 策略处理，或直接用自研编码器）。
  > 🔴 **2026-09-23 口径订正（杜先生裁定 ④）**：原文「GDSII/OASIS 属 A 级，永不借」把**格式**与**格式的库实现**混为一谈，且与本仓实际行为矛盾——`lda/lda_l2/gds_export.py` 是**自研编码器**，导出的是**真 GDSII**（DBU=1nm、AREF 层次化，`examples/README.md` 称「每个版图都是真实 GDSII」）。
  > 正确口径 = **禁 import 商业/第三方读写库；格式本身是公开事实标准，由自研实现兼容**。这正是「**标准可借鉴、IP / 技术不可引用**」在格式层的投影。
