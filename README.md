...
> 
> 当前版本：**v0.9.19**（2026-09-02 · **B15 波导光栅严格求解器接线 · 严格独立 13 → 14**：v0.9.18 曾判 B15「tmm.py 垂直入射多层膜与波导光栅物理模型错配 → 不接」；本轮推翻该前提——**写正确的求解器本身就是解法**：新写 `lda_solver/bragg_solver.py`，把 E(z)=n_eff²·(1+m·cos(2πz/Λ)) 周期调制波导的**反周期 Bloch 广义本征值问题** A ψ=β²B ψ（反周期边界 ψ(z+Λ)=−ψ(z) 锁定 k=±π/Λ，谱最低简并对=第一 Bragg 带隙上下沿，中心 → λ_B=2π/β_c）作为独立候选，与 golden 一阶相位匹配闭式 λ_B=2·n_eff·Λ **物理同源、方法独立**（golden=运动学相位匹配 vs cand=动力学全波本征谱，调制深度 m 进入算子）；严格独立 13→14、自证桩 34→33、降级仍 1（E2）；tol 未动（0.01，baseline 残差 8.36e-6 余量 1196×；反向 n_eff×1.1 信号 1.55e-1 余 15.5×）；网格 N=240 双向标定（N=480 偶然抵消点 5.4e-8、N=960 越 LAPACK 地板反升，均避开）；无调制极限自校（简并对劈裂 3.7e-13~2.7e-11 机器精度、β_c 命中 π/(Λ·n_eff)）；⚠️ 诚实边界：独立 14/48、自证桩 33）·
