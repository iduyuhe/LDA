...
def main() -> int:
    ...
    check("S9 锚：题库 50 题（B30+E7+S1-S13，含 S9/S10/S11/S12/S13）",
          "S9" in BENCHMARK_ORDER and len(BENCHMARK_ORDER) == 50,
    ...
    check("S10 锚：consistent=1.0 / 四反例 0.0（经 golden_value）",
          ...
          and all(golden_value("S10", {"case": c}) == 0.0
                  for c in ("cross_short", "via_short", "port_short",
                            "dangling")))
    check("S10 锚：题库 50 题（B30+E7+S1-S13，含 S9/S10/S11/S12/S13）",
          "S10" in BENCHMARK_ORDER and len(BENCHMARK_ORDER) == 50,
          f"n={len(BENCHMARK_ORDER)}")
    s10r = s10_report()
