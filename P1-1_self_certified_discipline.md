...
> 配套 B-3 收官 + B16 重审（v0.9.80）+ B567（v0.9.81，84 锚 / 严格独立 62/84 = 73.8%）。
> 本文件把**剩余 19 道自证桩**从「B-3 冲刺对象」转为**常态化工程纪律**，并订正此前过时口径。
...
- **B-3 专项已闭项 + B16 重审收官（v0.9.80）+ B5/B6 升严格独立（v0.9.81）**：84 锚、严格独立 **62** 道、降级 3（B21/E9/E10）、自证桩 **19**，独立率 **73.8%**（62/84）正式达标。
- 19 道自证桩**不再**作为一次性 sprint 目标，改为常态化纪律约束：任何触及它们的 PR 必须走「接独立候选 / 显式锁定」二选一。
...
## 3. 19 道处置映射（权威，harness 推导 · v0.9.81 刷新）

| 桶 | 锚（共 19） | 锁类型 | 升级路径 | 是否「必接候选」范围 |
|---|---|---|---|---|
...
- **self_certified 占比 22.6%（19/84）是真实短板但已持续缓解**：B-3 前 31.0%（22/71）→ B-3 后 26.2%（22/84）→ B16 重审后 25.0%（21/84）→ **B567 后 22.6%（19/84）**，且余下 6 道真候选均有明确路径，**无「伪绿」**。
