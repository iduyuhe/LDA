"""D-84/D-85 · 3D adjoint 形状逆设计统一入口（破 3D 诚实边界）。

D-84 平板波导宽度曲线（`design_shape3d`）+ **D-85 截面形状**
（`design_section3d`：宽度 w(x) × 厚度 h(x) 双软边界联合）。死标量验收：
  (a) 3D adjoint 梯度 FD 对拍 ≤0.15；
  (b) 形状梯度链式 FD 对拍 ≤0.15（宽度/截面）；
...
def main() -> int:
    import argparse
    ap = argparse.ArgumentParser(description="D-84/85 3D adjoint 形状逆设计")
    ap.add_argument("--mode", default="shape", choices=["shape", "section"])
