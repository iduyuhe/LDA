...
断言维度（全部死标量，LLM 不进判决路径）：
  ...
  3. 题库：BENCHMARK_ORDER 53 题（B1-B30 + B33 共 31 题 + E1-E9 9 题 + S1-S13 13 题）
...
class CountConsistencySmoke(unittest.TestCase):
    ...
    def test_benchmark_b30_e9_s13_split(self):
        ...
        # v0.9.67 新增 B33（探测器 RC 带宽严格独立锚）→ B 题 30→31；
        # B31/B32 未启用（编号跳号），末位为 B33，故末位断言取 B33。
        self.assertEqual(len(b_ids), 31, f"B 题应 31（B1-B30 + B33），实际 {len(b_ids)}")
