...
> 
> 当前版本：**v0.9.18**（2026-09-02 · **S13 设计良率锚接线 · 严格独立 12 → 13**：延续 P0 接线段，把 S13（环形 FSR 设计良率 DFY）接成真独立候选——golden=蒙特卡洛仿真良率、candidate=解析高斯积分闭式（Φ 精确解，保留 1/L 非线性），两种独立算法互证；严格独立 12→13、自证桩 35→34、降级仍 1（E2）；tol 未动（0.01，baseline 残差 3.4e-4 余量 29.7×）；⚠️ 诚实边界：S7/S8 解析均值是硬编码常量且对工艺容差 σ 不敏感（均值定理）→ 接独立候选必成伪独立（反向扰动信号=MC 涨落恒被 tol 吞没），已实测证否不接；B15 的 tmm.py 是垂直入射多层膜、与波导光栅一阶条件物理模型错配，留 v0.9.19）·
