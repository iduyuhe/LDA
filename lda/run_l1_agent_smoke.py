...
def main() -> int:
    ...
    # 1) reference → 21/21（物理定律 + 实证锚双 ground）
    r = run("verify_design", {"candidate": {"type": "reference"}})
    s = r.result["summary"]
    check("verify_design(reference) 21/21",
          r.status == "ok" and s.get("passed") == s.get("total") == 21,
          f"{s.get('passed')}/{s.get('total')} PASS")

    # 2) perturbed(rel=0.10) → 抓 FAIL（6/21，死标量）
    ...
    # 4) list_benchmarks → 21 题（B18 + E3）
    r = run("list_benchmarks", {})
    bm = r.result.get("benchmarks", [])
    ids = [b.get("id") for b in bm] if bm else []
    check("list_benchmarks 21 题",
          len(ids) == 21 and "B18" in ids and "E3" in ids,
          f"{len(ids)} 题（B1-B18 + E1-E3）")
