...
LDA 用开源、主权、零外部依赖的引擎，对标杆器件（GP-*）与整芯片（GC-*）完成规格驱动再设计，复现性能与公开 golden 死标量一致（7/7 PASS）。这证明：在不进入发动期、不实际流片的前提下，即可把验证做到产品级——以他人已量产/已验证的真实效果为外部尺子，杀同源自证风险，并为生态播种提供硬核素材。
