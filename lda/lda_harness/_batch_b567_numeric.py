...
# ===========================================================================
# B7 · 波导交叉串扰（**本批不接线** —— golden 侧待修）
# ===========================================================================
def crossing_crosstalk_dB(w_core: float, gap: float, wl: float,
                          n_si: float, n_clad: float) -> float:
    """90° 交叉串扰(dB) —— 双波导超模/CMT 模型（方法学独立于 2D FDTD）。

    物理：gap 相隔的两臂在交叉区构成耦合长度 L_eff 的耦合段，
    耦合系数 κ = π·(n_even − n_odd)/λ（超模拍），串扰 = sin²(κ·L_eff)。
    其中 n_even/n_odd 由双芯横向剖面 Helmholtz 本征解**严格**给出。

    ⚠️⚠️ 本函数**未接线**（B7 仍为 design_rule_anchor 自证桩）。原因见 `note`
    与 `P1-1_B567_report.md`：B7 的 golden 由 `oracle_field._fdtd2d_crossing`
    的离线 2D FDTD 给出（默认 -19.73 dB），经四路独立诊断**不收敛**：
      · 域尺寸 8/12/16µm ⇒ −19.73 / −13.25 / −12.43（|E|² 法，漂移 7 dB）
      · 网格 50/40/25nm ⇒ −19.73 / −17.59 / −14.85（漂移 5 dB）
      · 监视器距离 1.5–3.5µm ⇒ −11.3 ~ −20.7（±5 dB 乱跳）
      · 时间步 1400/2000/3000 ⇒ −19.73 / −17.25 / −15.00（未稳态）
      · 根因：吸收层强度 sig_max=0.06 相对 dt≈0.034 **弱约 100×**（实测 S≥1.0
        才饱和），且「cross 口」用单点 Σ|E|² 测的是交叉区**近场辐射**（随距离
        衰减）而非垂直波导的导模功率 ⇒ 度量本身在 2D 下不良定义。
    本 CMT 模型在自然 L_eff = w_core 下给 **−38.5 dB**，与 golden 的 −19.7 相差
    13 dB ≫ tol=5.0 ⇒ **在当前 golden 下无法诚实升格**，故不登记候选。
    """
    w = float(w_core)
    d = w + float(gap)
    x = _grid(8.0, 0.01)
