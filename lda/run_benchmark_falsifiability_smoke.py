...
PERTURB_SPEC = [
    ...
    ("B29", "dn_dt", "mul"),
    ("B30", "nbar", "mul"),
    # v0.9.67（A 档有源扩展 #1）：B33 探测器 RC 带宽（梯形法时域拟合 ↔ 解析闭式）。
    # 🔴 B33 扰 R（信号 |Δ|≈2.9e8 Hz @ tol 4e3，7.2e4× tol，1% 即抓）；
    ...
    ("B41", "L", "mul"),
    # v0.9.105（B-23 · 腿① 扩基）：B374-B386 高斯光束旁轴光学族 13 道严格独立新锚。
    # 🔴 反向键选择（信号 |Δ| vs golden 实测，只列 10% 扰动真能抓的键）：
    #   B374 zR：扰 w0（zR∝w0²，Δ≈0.21·zR=1.06e-5 ≫ tol 5e-7）✅
    #   B375 w(zR)：扰 w0（w∝w0，Δ≈0.1·w=7e-7 ≫ tol 5e-8）✅
    #   B376 共焦 b：扰 w0（b=2zR∝w0²）✅
    #   B377 θ红：扰 w0（θ∝1/w0，Δ≈0.091·θ=8.97e-3 > tol 5e-3，1.79×）✅
    #   B378 q腰：扰 f（f=zR，Δ≈0.164µm=1.64e-7 > tol 1e-7，1.64×）✅
    #   B379 q zR'：扰 f（Δ≈2.4µm=2.4e-6 > tol 1e-6，2.4×）✅
    #   B380 焦腰位 s：扰 f 仅 1.15e-7<tol 抓不住，改扰 w0（Δ≈4.8e-6 ≫ tol 5e-7，9.6×）✅
    #   B381 Gouy：扰 z2（Δ≈0.024 > tol 1e-2，2.4×）✅
    #   B382 w(2zR)：扰 w0（Δ≈2.2µm ≫ tol 5e-8）✅
    #   B384 w(zR/2)：扰 w0 ✅  · B385 w(3zR)：扰 w0 ✅
    #   B386 q腰(w0=8)：扰 f（Δ≈0.26µm=2.6e-7 > tol 1e-7，2.6×）✅
    #   B383 绿光发散角：与 B377 同候选同 golden 形式（仅波长绿），B377 已反向覆盖；
    #     10% 扰动下信号仅 0.73× tol 不单列（不造假绿，方法独立性由 B377 覆盖）。
    ("B374", "w0", "mul"),
    ("B375", "w0", "mul"),
    ("B376", "w0", "mul"),
    ("B377", "w0", "mul"),
...
#   口径由 30 回到 31。**当前：严格独立 63 · 降级 3(E9+E10+B21) · 自证桩 18 · 和 84**（v0.9.80 B16 重审、v0.9.81 B5/B6 各接第二独立求解器、v0.9.82 B7 golden 语义订正升严格独立）。
# v0.9.105（B-23）：高斯光束旁轴光学族 13 道严格独立锚（B374-B386）落地 ⇒ 严格独立 370 → 383，题数 391 → 404（降级 3、自证桩 18 不变）。
# v0.9.106（B-24）：标量衍射族 13 道严格独立锚（B387-B399）落地 ⇒ 严格独立 383 → 396（降级 3、自证桩 18 不变）。
# v0.9.108（B-26）：单界面 Fresnel/Snell 光学族 13 道严格独立锚（B413-B425）落地 ⇒ 严格独立 409 → 422（降级 3、自证桩 18 不变）。
# v0.9.107（B-25）：静电/静磁有限源族 13 道严格独立锚（B400-B412）落地 ⇒ 严格独立 396 → 409（降级 3、自证桩 18 不变）。
#   🔴 B383（绿光发散角）与 B377（红光发散角）共用同一候选 bpm_divergence_fd + 同一 golden 形式 golden_b377，仅波长不同；
#      B377 已反向测试（扰 w0 信号 1.79× tol 抓到）；B383 因绿光 θ 量值更小（0.0403 rad vs 红光 0.0987 rad），
#      10% 扰动下信号仅 0.73× tol 不可抓 ⇒ 不单列反向项（方法独立性由 B377 覆盖），不造假绿（反向测试只列真能抓的键）。
# v0.9.109（B-27）：色散与群速度族 13 道严格独立锚（B426-B438）落地 ⇒ 严格独立 422 → 435（降级 3、自证桩 18 不变）。
# v0.9.110（B-28）：不完全 Beta 函数族（B439-B445）+ 积分正余弦函数族（B446-B451）共 13 道严格独立锚落地
#   ⇒ 严格独立 435 → 448（降级 3、自证桩 18 不变）。
# 🔴 v0.9.110：MIN_INDEPENDENT 由 396（B-24 遗留低地板，B-25/B-26/B-27 三轮未同步上调）抬至 448
#   （B-28 严格独立真实基线）——本常量是**棘轮地板**，只应随接线单调上调；
#   停在 396 时，448 道里任意 52 道退化为自证桩都不会红，与本护栏「背离自证桩」的立项意图相悖。
MIN_INDEPENDENT = 448
