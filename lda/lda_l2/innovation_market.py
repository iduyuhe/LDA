...
# ===========================================================================
# 分类体系（v0.9.42 · 分类从前端 JS 下沉到数据层）
#
# 背景：store.html 曾硬编码一套目录分组，只覆盖 58/75 条，17 条新增货架用户
#       在目录视图里看不到。根因是「扩货架只改数据层，不改消费方」。
# 解法：分类存数据层，前端改由 /api/shelf 的 facets 动态渲染；新增货架漏标签
#       ⇒ run_shelf_taxonomy_smoke.py 直接红，从根上不可能再漏。
#
# 🔴 同步纪律（四处）：① .py 枚举与字段 ② .json 派生表 ③ 前端（已改为动态）
#    ④ CI 断言。见 run_shelf_taxonomy_smoke.py。
# ===========================================================================

#: 一级：赛道（用户心智里最自然的一级分类）
SHELF_TRACKS: Dict[str, str] = {
    "datacom":   "数通与电信",
    "sensing":   "传感与测量",
    "quantum":   "量子",
    "cpo":       "CPO 光 I/O",
...
#: 二级：应用域（赛道内细分；key 全局唯一，防 facets 计数跨赛道合并）
SHELF_APP_DOMAINS: Dict[str, Dict[str, str]] = {
    "datacom": {
        "transceiver": "收发引擎",
        "wdm_roadm":   "WDM 与 ROADM",
        "pon":         "接入网 PON",
        "coherent":    "相干与长距",
        "switching":   "光交换与路由",
    },
    ...
    "cpo": {
        "cpo_engine":      "CPO 光引擎",
        "chiplet":         "chiplet 与中介层",
        "optical_switch":  "OCS 光交换",
        "optical_compute": "光计算与新型互连",
    },
    "component": {
        "light_source": "光源与频梳",
        "modulation":   "调制与开关",
...
def taxonomy_check(track: str, app_domain: str) -> str:
    """校验 (track, app_domain) 组合合法；返回 "" 表示合法，否则返回原因。

    UI / API / CI 共用同一份判据，防出现「三处各判一套」的口径漂移。
    """
    if track not in SHELF_TRACKS:
        return f"未知 track={track!r}"
    if app_domain not in SHELF_APP_DOMAINS.get(track, {}):
        return (f"app_domain={app_domain!r} 不属于 track={track!r}"
                f"（允许：{sorted(SHELF_APP_DOMAINS.get(track, {}))}）")
    return ""
...
DEFAULT_SHELF: List[ShelfItem] = [
    ...
    # —— v0.8.38 货架库扩展（5→20）：市场调研信号驱动，仍严守组合已锚定基元护栏 ——
    #   信号源全部可溯源（IEEE/ITU 标准 / 厂商公开 roadmap / 公开文献 / 公开技术报告）；
    ...
    ShelfItem(
        ...
        signal_ref="Hyperphotonix Hyper Silicon™ 公开平台（400G DR4/800G DR8/1.6T DR8 PIC 路线）；"
                   "IEEE 802.3 800G 光接口标准进程",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-CROSSING"],
        default_req={"n_channels": 8, "channel_spacing_ghz": 100, "filter_bw_ghz": 40,
                     "link_budget_db": 4.5, "p_tx_dbm": 0, "wg_length_cm": 2.0},
        design_note="8×100G 并行单波长方案（DR8）：每通道 = 光栅 + 2cm SiN + crossing，"
                    "链路预算锚 S1/S2/S5/S7 死标量判决。对标 GC-DR4-TX/ONCHIP 公开规格量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-WDM-8CH-1D",
        ...
        signal_ref="IEEE 802.3bs 400GBASE-FR8/LR8（8 波 WDM 信道 IL 6.3 dB 上限，公开标准）；"
                   "AWG 40ch datasheet 量级参照",
        domain="photon",
        system_type="wdm_demux",
        composition=["GP-GRATING-EFF", "GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"n_channels": 8, "spacing_nm": 1.2},
        design_note="8 通道解复用：wdm_demux 闭环（B4：drop IL≤3 / XT≥15 / 单 FSR / DRC）。"
                    "对标 LR8 信道预算 6.3 dB（GC-LR8-CH 同源公开标准）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-DWDM-40CH",
        ...
        signal_ref="Qualinet/NTT-ID 40ch 100GHz Athermal AWG 公开 datasheet（插损 typ 4.5/max 6.0 dB）；"
                   "ITU-T G.694.1 DWDM 网格标准",
        domain="photon",
        system_type="wdm_demux",
        composition=["GP-GRATING-EFF", "GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"n_channels": 40, "spacing_nm": 0.8},
        design_note="40 通道 DWDM 解复用：wdm_demux 闭环（B4 锚）演示大规模波长数扩展。"
                    "对标 GC-AWG-40CH（商用 AWG datasheet 6.0 dB 死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-FTTH-PLC8",
        ...
        signal_ref="ITU-T G.671 / Telcordia GR-1209 公开典型最大插损 1×8 ≤10.7 dB；"
                   "ITU-T G.984.3 GPON ODN 预算标准",
        domain="photon",
        system_type="link",
        composition=["GP-YBRANCH", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 10.7, "p_tx_dbm": 3, "wg_length_cm": 1.0},
        design_note="1×8 = 3 级 Y-branch 级联 + SiN 波导；对标 GC-PLC-1X8（G.671 死标量）。"
                    "PON 上行突发时序按黑箱，判决只认链路预算锚 S1/S2/S5/S7。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-FTTH-PLC16",
        ...
        signal_ref="ITU-T G.671 / Telcordia GR-1209 公开典型最大插损 1×16 ≤14.0 dB；"
                   "商用 PLC datasheet 实测一致性（LuLeey ≤14.0 dB）",
        domain="photon",
        system_type="link",
        composition=["GP-YBRANCH", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 14.0, "p_tx_dbm": 3, "wg_length_cm": 1.0},
        design_note="1×16 = 4 级 Y-branch 级联；对标 GC-PLC-1X16（G.671 死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-CPO-OCS",
        ...
        signal_ref="UC Berkeley EECS-2024-213：Polatis 576×576 中位 1.4/max 3 dB、Google 136×136 ≤2 dB；"
                   "arXiv 2411.01503：2×FR4 功率预算 4.0 dB 公开",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 4.0, "p_tx_dbm": 3, "wg_length_cm": 1.0},
        design_note="OCS 收发前端：光栅 + SiN 波导，交换矩阵/MEMS 按黑箱（非片上器件）。"
                    "对标 GC-OCS-FABRIC（2×FR4 预算 4.0 dB 公开死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-LIDAR-TX",
        ...
        signal_ref="Optics Express 34, 7415 (2026)：片上 FMCW 单方向全光链路 ≈3.3 dB 实测；"
                   "Pointcloud Nature 2026 纯固态 FMCW 焦平面阵列公开路线",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 3.3, "p_tx_dbm": 0, "wg_length_cm": 0.5},
        design_note="FMCW 发射前端：光栅 + 0.5cm SiN（相干混频/OPA 按黑箱）。"
                    "对标 GC-LIDAR-FMCW（OE 2026 实测 3.3 dB 死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-QKD-TX-SHELF",
        ...
        signal_ref="npj Quantum Information 3, e1700262 (2017)：Alice 芯片总插损 15 dB 实测；"
                   "中国 QKD 干线（京沪干线）公开路线",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 15.0, "p_tx_dbm": 0, "wg_length_cm": 1.0},
        design_note="Alice 态制备 = 2×光栅 + 2×Y-branch（MZI）+ SiN；单光子衰减/调制按黑箱。"
                    "对标 GC-QKD-TX（npj QI 实测 15 dB 死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-QKD-RX-SHELF",
        ...
        signal_ref="npj Quantum Information 3, e1700262 (2017)：Bob 芯片总插损 8 dB 实测；"
                   "OE 28, 18449 (2020) 多用户接收机 13 dB 公开",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-CROSSING", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 8.0, "p_tx_dbm": 0, "wg_length_cm": 0.5},
        design_note="Bob 基矢测量 = 2×光栅 + crossing + SiN；单光子探测器按黑箱。"
                    "对标 GC-QKD-RX（npj QI 实测 8 dB 死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-QKD-MULTI4",
        ...
        signal_ref="Optics Express 28, 18449 (2020)：4 用户 MZI 选路接收机总损耗 13 dB 实测（公开）",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 3.0, "p_tx_dbm": 0, "wg_length_cm": 5.0},
        design_note="4 用户接收 = 3×光栅 + 5cm SiN（MZI 选路按黑箱）；SPD 按黑箱。"
                    "判决用标准链路余量锚 S1/S2/S5/S7（要求 3 dB）；整芯片 13 dB 总插损"
                    "对标见 GC-QKD-MULTI（OE 2020 实测死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-SENS-MZI",
        ...
        signal_ref="公开 PICS/FBG 传感链路综述：干涉型传感前端全链路插损预算通常 ≤15 dB"
                   "（商用光纤传感模块 10–18 dB 区间）",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 3.0, "p_tx_dbm": 0, "wg_length_cm": 2.0},
        design_note="MZI 传感 = 2×光栅 + 2×Y-branch（分/合束）+ 2cm SiN 双臂；传感元件按黑箱。"
                    "判决用标准链路余量锚 S1/S2/S5/S7（要求 3 dB）；整芯片全链路 ≤15 dB "
                    "预算对标见 GC-SENSE（公开传感链路预算区间）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-CHIPLET-IO",
        ...
        signal_ref="Broadcom 公开 CPO 路线（TH5-Bailly 6.4T 引擎，XPU 光连接演示）；"
                   "OIF CPO 2.0 公开路线图",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-CROSSING"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 6.0, "p_tx_dbm": 3, "wg_length_cm": 1.0},
        design_note="XPU 光 IO 前端 = 光栅 + SiN + crossing（EIC/PIC 键合按黑箱）。"
                    "对标 GC-CPO-8CH 同源（CPO 每通道 6–12 dB 公开区间）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-QCTRL-ZC3-10Q",
        ...
        signal_ref="上海科技情报研究所公开对比表：电子科大祖冲之三号 (2024, 105 qubit) "
                   "读出保真度 99.18%；D-46×D-47 已验证保真度预算框架",
        domain="hybrid",
        system_type="quantum_fidelity",
        composition=["GP-YBRANCH", "GP-SIN-PL"],
        default_req={"f01s": [5.0, 5.2, 5.4, 5.6, 5.8, 6.0, 6.2, 6.4, 6.6, 6.8]},
        design_note="10 比特复用读出链（D-46 复用 + D-47 保真度）。对标 GC-QCTRL-ZC3"
                    "（公开 99.18% 死标量），演示读出链规模扩展零新物理。",
        ci_status="",
    ),
    ShelfItem(
        ...
        signal_ref="上海科技情报研究所公开对比表：IBM Heron R2 (2024, 156 qubit) "
                   "读出保真度 98.5%；IBM Quantum Cloud 公开 readout error ~1%",
        domain="hybrid",
        system_type="quantum_fidelity",
        composition=["GP-YBRANCH", "GP-SIN-PL"],
        default_req={"f01s": [round(4.6 + 0.2 * i, 2) for i in range(16)]},
        design_note="16 比特复用读出链；对标 GC-QCTRL-HERON（公开 98.5% 死标量）。"
                    "156 比特整芯片按 heavy-hex 分段，本货架为单段代表。",
        ci_status="",
    ),
    ShelfItem(
        ...
        signal_ref="Applied Quantum 公开技术分析：Google Willow (2024, 105 qubit) "
                   "复用色散读出 + JPA，读出保真度 ~99.3%；arXiv 公开架构文档",
        domain="hybrid",
        system_type="quantum_fidelity",
        composition=["GP-YBRANCH", "GP-SIN-PL"],
        default_req={"f01s": [round(4.6 + 0.2 * i, 2) for i in range(12)]},
        design_note="12 比特复用读出链；对标 GC-QCTRL-WILLOW（公开 99.33% 死标量）。"
    ...
    ShelfItem(
        ...
        signal_ref="IEEE 802.3bm 100GBASE-PSM4（4×25G，500m SMF）公开标准；商用 PSM4 平台 datasheet 单通道插损 ≤4.0 dB",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-CROSSING"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 2.0, "link_budget_db": 4.0},
        design_note="PSM4 单通道 = 光栅 + 2cm SiN（边缘耦合低损）+ crossing；链路预算锚 S1/S2/S5/S7 死标量判决。对标 GC-PSM4-CH（IEEE 802.3bm 4.0 dB 死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-FR4-SHELF",
        ...
        signal_ref="IEEE 802.3bs 400GBASE-FR4（clause 121）单通道插损预算 ≤4.5 dB；Hyperphotonix 平台同量级",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 4.5},
        design_note="FR4 单通道 = 光栅 + 1cm SiN；WDM 复用/串行器按黑箱（非片上器件），判决复用 link 系统预算锚死标量。对标 GC-FR4-CH（IEEE 802.3bs 4.5 dB 死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-CWDM4-SHELF",
        ...
        signal_ref="CWDM4 MSA（100G CWDM4：4×25G，2km）单通道插损 ≤4.0 dB；商用 100G CWDM4 光模块 datasheet 一致",
        domain="photon",
        system_type="wdm_demux",
        composition=["GP-GRATING-EFF", "GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"n_channels": 4, "spacing_nm": 1.2},
        design_note="4 通道 CWDM 解复用：wdm_demux 闭环（B4：drop IL≤3 / XT≥15 / 单 FSR / DRC）。对标 GC-CWDM4-CH（CWDM-MSA 4.0 dB 死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-LPO-112G",
        ...
        signal_ref="LPO（线性可插拔光模块）公开产业路线（业界 112G/通道 线性直驱，去 Retimer/DSP 降功耗）：链路预算量级与 FR4/DR 同源",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 4.0},
        design_note="LPO 线性直驱前端 = 光栅 + SiN 波导（去 DSP 后链路裕度收窄，链路预算锚 S1/S2/S5/S7 死标量判决）；无量级新物理，复用已锚定基元。对标公开 LPO 链路预算量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-1.6T-DR8",
        ...
        signal_ref="1.6T DR8（16×100G PAM4）公开产业路线（OIF / 光模块厂商 1.6T DR8 MSA 量级）：单通道 100G PAM4，链路预算与 800G DR8 同源、通道翻倍",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-CROSSING"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 4.0},
        design_note="1.6T DR8 = 800G DR8 的 16 通道扩展（composition 同源 GP-*），复用已锚定无源网 + 光栅耦合死标量；无量级新物理。对标公开 1.6T DR8 链路预算量级。",
        ci_status="",
    ),
    # —— v0.8.48 货架扩容（25→33）：真实市场缺口品类，仍严守组合已锚定基元护栏 ——
    #   信号源全部可溯源（IEEE/ITU 标准 / LightCounting·Cignal AI·QYResearch 市场数据 /
    #   工信部试点公开报道 / 厂商公开路线）；system_type ∈ {link, wdm_demux}，判决复用已验证
    ...
    ShelfItem(
        ...
        signal_ref="OIF/光模块厂商 800G FR4（4×200G PAM4）公开路线：2026 均价 $400–480（硅光方案）；"
                   "LightCounting 2026 高速数通市场 ~$12B、800G+ 出货 6300 万只（2.6×）；"
                   "IEEE 802.3dj（200G/lane PAM4）预计 2026 中定稿",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-CROSSING"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.5, "link_budget_db": 4.5},
        design_note="800G FR4 = 4×200G PAM4 并行（与 800G DR8 同源无源网 + 光栅耦合，"
                    "composition 同源 GP-*）；200G/lane DSP+SiPh 2026 成熟。对标公开 800G FR4 "
                    "链路预算量级（单通道 ≤4.5 dB 量级）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-1.6T-FR4",
        ...
        signal_ref="1.6T FR4（4×400G PAM4）公开产业路线：NVIDIA GB300 标配、2026 量产拐点"
                   "（H1 主力上行）；硅光方案 $1000–1100；对标 1.6T DR8 同源、通道数减半",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-CROSSING"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 4.5},
        design_note="1.6T FR4 = 1.6T DR8 的 4×400G 变体（composition 同源 GP-*），复用已锚定"
                    "无源网 + 光栅耦合死标量；无量级新物理。对标公开 1.6T FR4 链路预算量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-400G-DR4",
        ...
        signal_ref="IEEE 802.3bs 400GBASE-DR4（4×100G，500m SMF）公开标准；"
                   "QYResearch 2026 全球 400G 光模块市场 ~$11.3 亿、传统云负载仍高量；"
                   "商用 400G-DR4 平台单通道插损 ≤4.0 dB",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-CROSSING"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 2.0, "link_budget_db": 4.0},
        design_note="400G DR4 = 4×100G 并行单波长（与 800G DR8 同源无源网 + 光栅耦合，"
                    "composition 同源 GP-*）；链路预算锚 S1/S2/S5/S7 死标量判决。对标 IEEE 802.3bs "
                    "400G DR4 单通道 4.0 dB 量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-100G-LR4",
        ...
        signal_ref="IEEE 802.3cu-2021 100GBASE-LR4（4×25G LAN-WDM，10km）公开标准；"
                   "5G 前传/城域/企业网高量部署；商用 100G LR4 光模块单通道插损 ≤4.5 dB",
        domain="photon",
        system_type="wdm_demux",
        composition=["GP-GRATING-EFF", "GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"n_channels": 4, "spacing_nm": 1.2},
        design_note="4 通道 LAN-WDM 解复用：wdm_demux 闭环（B4：drop IL≤3 / XT≥15 / 单 FSR / DRC）。"
                    "与 CWDM4-SHELF 同属 4ch 解复用前端，但面向 10km 长距 LAN-WDM（vs CWDM4 2km）。"
                    "对标 IEEE 802.3cu 100G LR4 单通道 4.5 dB 量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-PON-50G",
        ...
        signal_ref="ITU-T G.9804（Higher Speed PON）50G-PON 标准 2021 发布、2023 增补对称型；"
                   "工信部 2025 首批 168 试点→2026 商用启航（2026.4 验收 136：52 小区/38 工厂/46 园区）；"
                   "功率预算 N1(29dB)/C+(32dB)、1:64 分光",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 6.0, "p_tx_dbm": 3, "wg_length_cm": 1.0},
        design_note="50G-PON 光前端 = 光栅耦合 + Y-branch 分束 + SiN 波导（OLT 发射/ONU 接收无源网）。"
                    "PIC 自身 IL 预算 6 dB；整系统 29/32 dB 功率预算由 ODN 1:64 分光主导（黑箱，"
                    "非片上器件），判决复用 link 系统预算锚 S1/S2/S5/S7。对标 G.9804 死标量量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-OSW-1X8",
        ...
        signal_ref="数据中心可重构光交换（OCS/dOCS）趋势：Cignal AI 预测 OCS 全球市场 2029 ≥$25 亿；"
                   "LightCounting 预计 Scale-Up 光互连 2027 规模商用；SiPh MZI 矩阵开关（8–72 端口，"
                   "0.5 dB/级）公开；Google TPU v4 9216 卡 OCS 互联",
        domain="photon",
        system_type="link",
        composition=["GP-YBRANCH", "GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 6.0, "p_tx_dbm": 3, "wg_length_cm": 1.0},
        design_note="1×8 光开关无源前端 = Y-branch/MMI 分束 + SiN 波导；MZI 热光开关矩阵按黑箱"
                    "（有源不物理级建模，负面清单）。判决复用 link 系统预算锚死标量。对标公开 SiPh "
                    "MZI 矩阵开关 0.5 dB/级量级。",
        ci_status="",
    ),
    ShelfItem(
        ...
        signal_ref="FMCW 激光雷达相干接收机公开路线（Aeva/Bosch 等全固态 FMCW）；与 IM-LIDAR-TX"
                   "（Optics Express 34,7415 (2026) 实测 3.3 dB）配套，90° 混频 + 平衡探测按黑箱；"
                   "相干探测公开链路预算量级",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-CROSSING"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 3.3, "p_tx_dbm": 0, "wg_length_cm": 0.5},
        design_note="FMCW 相干接收前端 = 光栅 + 0.5cm SiN + crossing（90° 混频/平衡探测按黑箱）。"
                    "与 IM-LIDAR-TX 同源无源网，链路预算锚 3.3 dB（OE 2026 实测死标量）。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-BIOSENSE",
        ...
        signal_ref="公开 Lab-on-Chip 环形谐振传感文献综述：微环折射率传感在生物/化学检测"
                   "（如多路免疫传感）的成熟应用；复用 SENSE-RING 拓扑与 GP-GRATING-EFF+GP-SIN-PL 基元",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 3.0, "p_tx_dbm": 0, "wg_length_cm": 1.0},
        design_note="环谐振生物传感前端 = 光栅 + 1cm SiN（环形谐振腔传感单元，复用 SENSE-RING 拓扑）。"
                    "传感灵敏度由环 Q / 波长偏移换算，属参数化下一迭代；判决复用 link 系统预算锚。",
        ci_status="",
    ),
    # —— v0.8.49 货架扩容（33→38）：真实 2026 市场缺口品类，仍严守组合已锚定基元护栏 ——
    #   信号源全部可溯源（OIF/IEEE 标准 / Research and Markets·Dell'Oro·Cignal AI·LightCounting
    #   市场数据 / 厂商公开路线 / 公开技术报告）；system_type=link（复用已验证闭环），零新物理。
    ...
    ShelfItem(
        ...
        signal_ref="OIF 800ZR 互操作 IA（2024-11 发布）公开；IEEE 802.3dj 1600ZR 预计 2026 中定稿；"
                   "Research and Markets：ZR+ 相干光模块市场 2025 $18.4 亿→2026 $21.9 亿（CAGR 18.9%）；"
                   "Dell'Oro/Cignal AI：800ZR/ZR+ 2026 进入大规模部署",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-CROSSING"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 6.0},
        design_note="400G ZR/ZR+ 相干收发 PIC 前端 = 光栅耦合 + 1cm SiN（低损）+ crossing；"
                    "相干 DSP / IQ 调制器 / 平衡探测按黑箱（有源不物理级建模，负面清单）。"
                    "PIC 自身 IL 预算 6 dB（非整系统 120km 链路，链路预算锚 S1/S2/S5/S7 死标量判决）。"
                    "复用已锚定无源网 + 光栅耦合，无量级新物理。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-RING-MOD",
        ...
        signal_ref="NVIDIA CPO 采用微环调制器（MRM，带宽密度 >1 Tbps/mm 公开路线）；"
                   "TSMC COUPE 2026 量产 200Gbps/lane MRM、带宽密度 0.5→4 Tbps/mm (2030)；"
                   "Ayar Labs TeraPHY / NewPhotonics 无热 MRM（OFC 2026 公开）",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 4.0},
        design_note="MRM 发射前端 = 光栅耦合 + 1cm SiN 波导；环形谐振调制单元（有源）按黑箱"
                    "（有源不物理级建模，负面清单），其谐振/调制行为由文献锚走完闭环。"
                    "判决复用 link 系统预算锚 S1/S2/S5/S7 死标量。复用已锚定无源网，无量级新物理。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-XGS-PON",
        ...
        signal_ref="ITU-T G.9807.1（XGS-PON，10G 对称）标准；Dell'Oro：PON 设备营收 $8.3B(2021)→$9.8B(2026)，"
                   "XGS-PON 占 PON 市场 15%(2021)→55%(2026)；中国 10G PON 端口 3201 万(2026-03)→3286 万(2026-06)；"
                   "Openreach 英国扩 XGS-PON",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"n_channels": 1, "channel_spacing_ghz": 100, "filter_bw_ghz": 50,
                     "link_budget_db": 6.0, "p_tx_dbm": 3, "wg_length_cm": 1.0},
        design_note="XGS-PON 光前端 = 光栅耦合 + Y-branch 分束 + SiN 波导（OLT 发射/ONU 接收无源网）。"
                    "PIC 自身 IL 预算 6 dB；整系统功率预算由 ODN 1:64 分光主导（黑箱，非片上器件），"
                    "判决复用 link 系统预算锚 S1/S2/S5/S7。对标 G.9807.1 量级（与 50G-PON 同族，"
                    "速率 10G 对称）。",
        ci_status="",
    ),
    ShelfItem(
        ...
        signal_ref="MarkWide：ROADM WSS 市场 $1.8B(2026)→$4.76B(2035) CAGR 11.4%；"
                   "Dual WSS $325M(2025)→$384.43M(2026)；支撑 400G/800G/1.6T 相干；Lumentum/Coherent 主导",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 6.0},
        design_note="1×9 WSS 无源前端 = 光栅耦合 + MMI 分束 + SiN 波导；波长选择/切换矩阵"
                    "（LCOS/MEMS）按黑箱（有源不物理级建模，负面清单）。判决复用 link 系统预算锚"
                    "死标量。复用已锚定无源网，无量级新物理。对标公开 ROADM WSS 量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-VOA",
        ...
        signal_ref="MEMS VOA 市场 $215.5M(2025)→$320.89M(2032) CAGR 5.85%；"
                   "Variable Optical Attenuators $380M(2025)→$551.7M(2032) CAGR 5.4%；"
                   ">70% 光网络用动态衰减（ROADM 功率均衡）",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 4.0},
        design_note="VOA 前端 = 光栅耦合 + 1cm SiN 波导；可变衰减单元（MEMS/热光）按黑箱"
                    "（有源不物理级建模，负面清单）。判决复用 link 系统预算锚 S1/S2/S5/S7 死标量。"
                    "复用已锚定无源网，无量级新物理。对标公开 VOA 量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-MZI-MOD",
        ...
        signal_ref="硅基 MZM 市场 $1.51B(2025)→$7.24B(2034) CAGR 19.0%（Growth Market Reports 2026-06）；"
                   "Tower Semiconductor+Coherent（2026-03）400 Gbps/lane 硅 MZM 量产就绪；"
                   "200+ GHz 带宽、sub-0.5 dB 插损；硅 MZM 占 2025 MZM 市场 45.8%。",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 2.0, "link_budget_db": 3.0},
        design_note="MZM 前端 = 光栅耦合 + 1×2 MMI 分束（两相位臂）+ SiN 波导臂 + 1×2 MMI 合束"
                    "（复用分束基元作合束）+ 光栅耦合出。电光调制相移单元按黑箱"
                    "（有源不物理级建模，负面清单）。判决复用 link 系统预算锚 S1/S2/S5/S7 死标量。"
                    "复用已锚定无源网，无量级新物理。对标公开硅 MZM 量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-PSR",
        ...
        signal_ref="Sama et al. Optics and Laser Technology 2026 vol 203，高隔离 PSR（SOI 220nm+70nm 部分刻蚀），"
                   "TM-to-TE 损耗 0.71 dB @1550nm、PER 最差 30.95 dB、C 波段；"
                   "偏振分束器市场 $1.2-1.5B(2025)→$2.2-3.2B(2035) CAGR 7-9%（IndexBox）。",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 3.0},
        design_note="PSR 前端 = 光栅耦合 + 非对称 Y 分支偏振路由 + SiN 波导；偏振旋转/耦合的"
                    "亚波长结构按黑箱（有源不物理级建模，负面清单）。判决复用 link 系统预算锚"
                    "死标量。复用已锚定无源网，无量级新物理。对标公开 PSR 量级。",
        ci_status="",
    ),
    ShelfItem(
        ...
        signal_ref="IDTechEx 预测 CPO 市场 2036 破 $20B、CAGR 37%；TSMC COUPE 2026-04 量产；"
                   "NVIDIA Quantum-X/Spectrum-X Photonics CPO 2026 出货；Ayar Labs TeraPHY $500M E 轮 2026-03；"
                   "2.5D 硅中介层+TSV 路径。",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-MMI-1X2", "GP-CROSSING", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 5.0, "link_budget_db": 3.0},
        design_note="光子中介层前端 = 光栅阵列 I/O + MMI 扇出 + 交叉矩阵 + Y 分支 + SiN 波导面；"
                    "TSV/微环驱动按黑箱（有源不物理级建模，负面清单）。判决复用 link 系统预算锚"
                    "死标量。复用已锚定无源网，无量级新物理。对标公开 CPO 光互连量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-OPTO-COMPUTE",
        ...
        signal_ref="光神经网络处理器市场 Lightmatter/Lightelligence/Celestial AI/Intel/Ayar Labs 占 56.3%"
                   "（Global Market Insights）；北美光子神经形态芯片 $180-240M(2026) CAGR 32-38%；"
                   "Lightmatter 1.2 petaflops 模拟 ONN、Lightelligence 8.3 pJ/op。",
        domain="photon",
        system_type="link",
        composition=["GP-MMI-1X2", "GP-CROSSING", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 3.0, "link_budget_db": 3.0},
        design_note="ONN 前端 = 1×2 MMI 分光权重分配 + 交叉干涉网格（MZI mesh）+ SiN 波导；"
                    "相移/探测按黑箱（有源不物理级建模，负面清单）。判决复用 link 系统预算锚"
                    "死标量。复用已锚定无源网，无量级新物理。对标公开光计算量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-OCT",
        ...
        signal_ref="OCT 市场 $2.36B(2026)→$4.01B(2032) CAGR 9.08%（Research and Markets）；"
                   "眼科 OCT $1.52B(2025)→$2.52B(2032) CAGR 6.49%；糖尿病视网膜病变+AMD 驱动。",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-MMI-1X2", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 2.0, "link_budget_db": 3.0},
        design_note="OCT 前端 = 光栅耦合 + 1×2 MMI 分光（样品/参考臂）+ Y 分支 + SiN 波导干涉臂；"
                    "扫描/探测按黑箱（有源不物理级建模，负面清单）。判决复用 link 系统预算锚"
                    "死标量。复用已锚定无源网，无量级新物理。对标公开 OCT 量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-OPA-LIDAR",
        ...
        signal_ref="OPA LiDAR 市场 Dataintelo $1.8B(2025)→$9.6B(2034) CAGR 20.4%；单芯片集成 OPA 占 63.7%（2025）；"
                   "Yole：投入 OPA 研发厂商>35 家、车规原型 12 家，2026 乘用车前装渗透率破 5%（~$4.2 亿）；"
                   "CMOS 兼容硅光波导型 OPA（MZI 阵列/级联光栅阵列）为主流。",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL", "GP-CROSSING"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 2.0, "link_budget_db": 3.0},
        design_note="OPA 前端 = 光栅发射阵列 + Y 分支功分馈入阵元 + SiN 波导 + 交叉路由；"
                    "相位调制单元按黑箱（有源不物理级建模，负面清单）。判决复用 link 系统预算锚"
                    "死标量。复用已锚定无源网，无量级新物理。对标公开 OPA LiDAR 量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-COHERENT-RX",
        ...
        signal_ref="Optical Hybrid 市场 $483.33M(2025)→$1.10B(2032) CAGR 12.48%；90° 光混频占 67.3%（2025）；"
                   "由 400G→800G→1.6T 相干可插拔驱动，SiPh+InP 集成降 footprint/功耗；"
                   "相干光通信占应用 75.9%。",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 2.0, "link_budget_db": 3.0},
        design_note="相干接收前端 = 光栅耦合（信号+本振双入）+ 1×2 MMI 4×4 混频（两 MMI 作分/合）+ SiN 波导；"
                    "平衡探测按黑箱（有源不物理级建模，负面清单）。判决复用 link 系统预算锚"
                    "死标量。复用已锚定无源网，无量级新物理。对标公开相干接收量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-ONCHIP-NOC",
        ...
        signal_ref="Chiplet 互连光子市场 $1.8B(2025)→$52.1B(2034) CAGR 38.5%（Market Intelo）；"
                   "光互连 $13.69B(2025)→$15.28B(2026) CAGR 11.6%→$23.54B(2030)；"
                   "Ayar TeraPHY（UCIe 光 chiplet 8Tbps）、Intel 光 I/O 4Tbps/5pJ/bit。",
        domain="photon",
        system_type="link",
        composition=["GP-CROSSING", "GP-YBRANCH", "GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 3.0, "link_budget_db": 3.0},
        design_note="ONoC 前端 = 交叉矩阵（路由） + Y 分支功分 + 1×2 MMI + SiN 波导 fabric；"
                    "调制/探测按黑箱（有源不物理级建模，负面清单）。判决复用 link 系统预算锚"
                    "死标量。复用已锚定无源网，无量级新物理。对标公开片上光互连量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-MCF-FANOUT",
        ...
        signal_ref="MCF Fanouts 市场 $640M(2025)→$1.25B(2032) CAGR 11.8%（Strategic Market Research）；"
                   "窄口径 $87.5M(2025)→$504.85M(2032) CAGR 28.45%；OFC 2026 SDM4 MCF MSA"
                   "（Corning/AFL/Sumitomo）4 芯；TPU 首条商用 MCF（2025-2026）。",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 3.0},
        design_note="MCF 扇出前端 = 光栅耦合阵列（多芯入） + Y 分支扇出到单芯 + SiN 波导；"
                    "多芯对准按黑箱（有源不物理级建模，负面清单）。判决复用 link 系统预算锚"
                    "死标量。复用已锚定无源网，无量级新物理。对标公开 MCF 扇出量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-OPTICAL-GYRO",
        ...
        signal_ref="FOG 市场 $1.2B(2026)→$2.0B(2033) CAGR 7.5%（Persistence）；或 $1.96B(2025)→$4.60B(2034) CAGR 9.93%；"
                   "干涉型 FOG（Sagnac 效应）占 78%（2026）；GNSS 拒止环境 + 国防现代化驱动。",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-CROSSING", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 2.0, "link_budget_db": 3.0},
        design_note="FOG 前端 = 光栅耦合 + 交叉环形（Sagnac 环） + Y 分支分/合 + SiN 波导；"
    ...
    ShelfItem(
        ...
        signal_ref="AWG MUX/DeMUX 市场 $735M(2025)→$1.375B(2031) CAGR 8.14%；Arrayed Waveguide "
                   "Market $320-570M(2026) CAGR 6.5-11.7%；QYResearch $270M(2025)→$427M(2032) "
                   "CAGR 6.9%；Thermal AWG $1.55B(2026)→$2.84B(2033) CAGR 9.1%；AI 数据中心 "
                   "DWDM/CPO 结构性推升需求。",
        domain="photon",
        system_type="link",
        composition=["GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 3.0},
        honest_tier="前瞻预研",
        design_note="AWG 解复用前端 = 输入/输出 MMI 星形耦合器（GP-MMI-1X2 近似） + SiN 阵列波导"
                    "（GP-SIN-PL 自由色散长度）；光源/探测按黑箱（有源不物理级建模，负面清单）。"
                    "判决复用 link 系统预算锚死标量。复用已锚定无源网，无量级新物理。对标公开 "
                    "AWG DeMUX 量级。",
        ci_status="",
    ),
    ShelfItem(
        ...
        signal_ref="Chip-scale Spectrometer $2.44B(2025)→$8.7B(2033) CAGR 17.2%；Chip/Modular "
                   "Spectrometers $546M(2026)→$886M(2032) CAGR 8.4%；Miniature Spectrometer IC "
                   "$1.36B(2025)→$3.99B(2034) CAGR 12.7%（医疗/环境/消费电子驱动）。",
        domain="photon",
        system_type="link",
        composition=["GP-MMI-1X2", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 3.0},
        honest_tier="前瞻预研",
        design_note="片上光谱仪前端 = MMI 分光（GP-MMI-1X2） + Y 分支路由阵列（GP-YBRANCH） + "
                    "SiN 波导色散/干涉臂（GP-SIN-PL）；探测器按黑箱（有源不物理级建模，负面"
                    "清单）。判决复用 link 系统预算锚死标量。复用已锚定无源网，无量级新物理。"
                    "对标公开微型光谱仪量级。",
        ci_status="",
    ),
    ShelfItem(
        ...
        signal_ref="Few-Mode Fibers $10.74B(2025) CAGR 6.86%；MDM Equipment $1.42B(2024)→$4.16B"
                   "(2033) CAGR 12.6%；Market Intelo →$6.89B CAGR 18.9%；Growth Market Reports "
                   "CAGR 13.7%（突破单模容量极限）。",
        domain="photon",
        system_type="link",
        composition=["GP-YBRANCH", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 3.0},
        honest_tier="前瞻预研",
        design_note="模分复用前端 = 非对称 Y 分支模式合/分器（GP-YBRANCH 模式转换近似） + SiN 少模"
                    "波导（GP-SIN-PL）；光源/探测按黑箱（有源不物理级建模，负面清单）。判决复用 "
                    "link 系统预算锚死标量。复用已锚定无源网，无量级新物理。对标公开 MDM 量级。",
        ci_status="",
    ),
    ShelfItem(
        id="IM-OPTCOMB",
        ...
        signal_ref="Intel Market Research $58M(2025)→$108M(2034) CAGR 7.4%；全球光频梳 $1.87B"
                   "(2026) 年增 31.7%；Archive Market Research $1.8B(2025) CAGR 11%；芯片级微梳 "
                   "CAGR 47.8%（DWDM/时频同步/量子）。",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 3.0},
        honest_tier="前瞻预研",
        design_note="光频梳前端 = 高效光栅耦合 IO（GP-GRATING-EFF） + SiN 微环梳谐振波导"
                    "（GP-SIN-PL，高 Q 色散工程）；泵浦/探测按黑箱（有源不物理级建模，负面"
                    "清单）。判决复用 link 系统预算锚死标量。复用已锚定无源网，无量级新物理。"
                    "对标公开芯片级微梳量级。",
        ci_status="",
    ),
    ShelfItem(
        ...
        signal_ref="光偏振控制器 $480M(2026) 增 12.3% / $4.72B(2026) 增 12.9%；Polarization "
                   "Rotator CAGR 10.3%(2026-2033)；Stats N Data CAGR 6%；集成波导型增速 28%"
                   "（硅光量产拉动）。",
        domain="photon",
        system_type="link",
        composition=["GP-CROSSING", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 3.0},
        honest_tier="前瞻预研",
        design_note="偏振旋转器前端 = 波导交叉路由（GP-CROSSING） + SiN 双折射/非对称波导偏振旋转"
                    "段（GP-SIN-PL）；光源/探测按黑箱（有源不物理级建模，负面清单）。判决复用 "
                    "link 系统预算锚死标量。复用已锚定无源网，无量级新物理。对标公开偏振旋转器"
                    "量级。",
        ci_status="",
    ),
    # —— M3（货架 58→70）+ M3+（70→71）：§2.1~2.3 拟货架（5 A + 4 B + 3 C；IM-OPTCOMB-WDM 差异化为 WDM 锁定微梳、M3+ 已独立上架）——
    ...
    ShelfItem(
        ...
        signal_ref="中际旭创/新易盛 1.6T&3.2T OSFP DR8 公开规格（单通道插损 ≤4.5 dB）；"
                   "Hyperphotonix 800G/1.6T DR8 平台；IEEE 802.3df 800G/1.6T 进程",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"n_channels": 8, "channel_spacing_ghz": 100.0,
                     "filter_bw_ghz": 50.0, "link_budget_db": 3.0, "p_tx_dbm": 6.0},
        design_note="3.2T DR8 前端 = 高效光栅耦合阵列（GP-GRATING-EFF）+ SiN 波导引出（GP-SIN-PL）；"
                    "8×400G PAM4 并行，调制器/激光器按黑箱（负面清单）。判决复用 link 系统预算锚"
                    "死标量；复用已锚定无源网，无量级新物理。对标公开 DR8 量级。",
        features=[
            "复用已验证 link 闭环（S1 功率预算 / S5 能量守恒下界 / S7 统计最坏情况）",
            "8×400G PAM4 并行通道，光栅+SiN 全锚定基元组装，零新物理",
            "调制器/激光器按黑名单处理，判决只认无源链路死标量",
        ],
        applications=["AI 数据中心 3.2T 光模块", "DR8 多通道并行互连", "CPO 光引擎通道"],
        specs={"通道数": "8（8×400G PAM4）", "单通道插损": "≤4.5dB（对标公开·仿真预期）",
               "判决锚": "link / S1·S5·S7"},
        peers=[{"vendor": "中际旭创", "product": "1.6T/3.2T OSFP DR8",
                "specs": {"单通道插损": "≤4.5dB"}, "note": "公开规格（仿真预期·前瞻预研·未流片验证）",
                "source": "中际旭创 2024 年报 / 公开产品规格"}],
        ci_status="",
    ),
    ShelfItem(
        id="IM-1.6T-LPO",
        ...
        signal_ref="新易盛/云晖 1.6T LPO 公开规格（单通道插损 ≤4.5 dB）；LPO 线性直驱 MSA 路线",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"n_channels": 8, "channel_spacing_ghz": 100.0,
                     "filter_bw_ghz": 50.0, "link_budget_db": 3.0, "p_tx_dbm": 6.0},
        design_note="1.6T LPO 前端 = 高效光栅耦合（GP-GRATING-EFF）+ SiN 波导引出（GP-SIN-PL）；"
                    "线性直驱省 retimer，调制器按黑箱。判决复用 link 系统预算锚死标量；"
                    "复用已锚定无源网，无量级新物理。对标公开 LPO 量级。",
        features=[
            "复用已验证 link 闭环（S1/S5/S7 死标量锚）",
            "8×200G 线性直驱通道，光栅+SiN 全锚定基元组装",
            "线性直驱省 retimer，调制器按黑名单处理",
        ],
        applications=["AI 数据中心 1.6T LPO 可插拔", "线性直驱低功耗互连", "交换机近封装"],
        specs={"通道数": "8（8×200G）", "单通道插损": "≤4.5dB（对标公开·仿真预期）",
               "判决锚": "link / S1·S5·S7"},
        peers=[{"vendor": "新易盛", "product": "1.6T LPO",
                "specs": {"单通道插损": "≤4.5dB"}, "note": "公开规格（仿真预期·前瞻预研·未流片验证）",
                "source": "新易盛公开产品规格 / 公告"}],
        ci_status="",
    ),
    ShelfItem(
        ...
        signal_ref="光迅/Acacia 相干 400ZR/1.6ZR 公开规格；OIF 400ZR/800ZR/1.6ZR 标准",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"n_channels": 4, "channel_spacing_ghz": 100.0,
                     "filter_bw_ghz": 50.0, "link_budget_db": 3.0, "p_tx_dbm": 6.0},
        design_note="1.6T ZR 前端 = 高效光栅耦合（GP-GRATING-EFF）+ SiN 波导引出（GP-SIN-PL）；"
                    "相干 DSP/调制器按黑箱。判决复用 link 系统预算锚死标量；"
                    "复用已锚定无源网，无量级新物理。对标公开相干 ZR 量级。",
        features=[
            "复用已验证 link 闭环（S1/S5/S7 死标量锚）",
            "相干 400ZR/1.6ZR 通道，光栅+SiN 全锚定基元组装",
            "相干 DSP/调制器按黑名单处理",
        ],
        applications=["DCI 相干互连", "1.6ZR 长距离传输", "城域/骨干相干收发"],
        specs={"通道数": "4（相干 1.6ZR）", "单通道插损": "≤4.5dB（对标公开·仿真预期）",
               "判决锚": "link / S1·S5·S7"},
        peers=[{"vendor": "光迅科技", "product": "相干 400ZR/1.6ZR",
                "specs": {"单通道插损": "≤4.5dB"}, "note": "公开规格（仿真预期·前瞻预研·未流片验证）",
                "source": "光迅科技公开 datasheet"}],
        ci_status="",
    ),
    ShelfItem(
        id="IM-CPO-16CH",
        ...
        signal_ref="OIF CPO 3.2T/6.4T 白皮书；CPO 共封装光学（Azure/Google/AWS）从试点转早期量产",
        domain="photon",
        system_type="wdm_demux",
        composition=["GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"n_channels": 16, "spacing_nm": 2.0},
        design_note="CPO 16 通道 WDM 前端 = MMI 星形耦合（GP-MMI-1X2）+ SiN 阵列波导（GP-SIN-PL）；"
                    "判决复用 wdm_demux 已验证闭环（B4 锚：drop IL≤3dB / XT≥15dB）。"
                    "复用已锚定无源网，无量级新物理。对标公开 CPO WDM 量级。",
        features=[
            "复用已验证 wdm_demux 闭环（B4：drop IL≤3dB / XT≥15dB）",
            "16 通道 @ 2.0nm WDM，MMI+SiN 全锚定基元组装",
            "光源/探测按黑名单处理，判决只认 WDM 死标量",
        ],
        applications=["共封装光学（CPO）WDM 解复用", "高密度片上光 IO", "AI 交换机共封装"],
        specs={"通道数": "16", "通道间隔": "2.0nm",
               "drop 插损": "≤3dB（B4 锚·仿真预期）", "串扰": "≥15dB（B4 锚·仿真预期）"},
        peers=[{"vendor": "Broadcom", "product": "TH5-Bailly 6.4T CPO",
                "specs": {"片上损耗": "量级 ≤12dB"}, "note": "公开 roadmap（仿真预期·前瞻预研·未流片验证）",
                "source": "Broadcom 公开 roadmap"}],
        ci_status="",
    ),
    ShelfItem(
        id="IM-UCIE-OPTICAL",
        ...
        signal_ref="UCIe 2.0 新增光互连（UCIe-Optical）标准草案；Intel/Ayarl 光 chiplet 公开路线",
        domain="photon",
        system_type="wdm_demux",
        composition=["GP-MMI-1X2", "GP-SIN-PL"],
        default_req={"n_channels": 8, "spacing_nm": 2.0},
        design_note="UCIe-Optical 前端 = MMI 星形耦合（GP-MMI-1X2）+ SiN 波导（GP-SIN-PL）；"
                    "判决复用 wdm_demux 已验证闭环（B4 锚）。复用已锚定无源网，无量级新物理。"
                    "对标公开光 chiplet 互连量级。",
        features=[
            "复用已验证 wdm_demux 闭环（B4：drop IL≤3dB / XT≥15dB）",
            "8 通道光 chiplet 互连，MMI+SiN 全锚定基元组装",
            "die 间耦合/探测按黑名单处理",
        ],
        applications=["die-to-die 光互连", "光 chiplet / ONoC", "异构集成封装内光互连"],
        specs={"通道数": "8", "通道间隔": "2.0nm",
               "drop 插损": "≤3dB（B4 锚·仿真预期）", "串扰": "≥15dB（B4 锚·仿真预期）"},
        peers=[{"vendor": "Intel", "product": "UCIe-Optical 光 chiplet",
                "specs": {"单通道插损": "≤4.5dB"}, "note": "公开路线（仿真预期·前瞻预研·未流片验证）",
                "source": "Intel 公开 roadmap / UCIe 2.0 草案"}],
        ci_status="",
    ),
    ShelfItem(
        id="IM-LIDAR-FULL",
        ...
        signal_ref="Optics Express 34, 7415 (2026) 公开论文：片上 FMCW LiDAR 单方向链路损耗 ≈3.3 dB；"
                   "FMCW 固态雷达产业（Aeva/Mobileye）公开路线",
        domain="photon",
        system_type="sensor_frontend",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"app": "lidar", "n_gratings": 2, "wg_length_cm": 0.5,
                     "n_ybranch": 0, "n_crossing": 0, "budget_db": 9.0, "tol_db": 2.0},
        design_note="FMCW LiDAR 全前端 = 光栅耦合（GP-GRATING-EFF）+ SiN 波导（GP-SIN-PL）"
                    "+ Y-branch 路由（GP-YBRANCH）；OPA 扫描/相干混频按黑箱（负面清单）。"
                    "判决复用 sensor_frontend 自带死标量锚 S1（预算）/S5（能量守恒下界·D-67）/S7（统计 p5）；"
                    "复用已锚定无源网，无量级新物理。对标公开 FMCW LiDAR 链路量级。",
        features=[
            "复用 sensor_frontend 闭环（S1 预算 / S5 能量守恒下界 / S7 统计 p5）",
            "TX+RX 一体 FMCW 前端，光栅+Ybranch+SiN 全锚定基元组装",
            "OPA 扫描/调制器/探测器按黑名单处理",
        ],
        applications=["车载/机器人 FMCW 固态 LiDAR", "光探测与测距全光前端", "同轴 TX/RX"],
        specs={"链路插损预算": "≤9dB（S1·仿真预期）", "判决锚": "sensor_frontend / S1·S5·S7",
               "复用基元": "GP-GRATING-EFF+GP-YBRANCH+GP-SIN-PL"},
        peers=[{"vendor": "Aeva", "product": "FMCW 固态 LiDAR",
                "specs": {"单方向链路": "≈3.3dB"}, "note": "公开论文量级（仿真预期·前瞻预研·未流片验证）",
    ...
    ShelfItem(
        ...
        signal_ref="公开 PICS / 生物光子传感综述：干涉型传感前端全链路插损预算通常 ≤15 dB；"
                   "POCT 生物传感产业（国内外 IVD）公开路线",
        domain="photon",
        system_type="sensor_frontend",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"app": "poc", "n_gratings": 2, "wg_length_cm": 2.0,
                     "n_ybranch": 2, "n_crossing": 0, "budget_db": 15.0, "tol_db": 3.0},
        design_note="POCT 传感前端 = 光栅耦合（GP-GRATING-EFF）+ Y-branch 分/合束（GP-YBRANCH）"
                    "+ SiN 波导传感臂（GP-SIN-PL）；传感元件按参数化黑箱（负面清单）。"
                    "判决复用 sensor_frontend 自带死标量锚 S1/S5/S7；复用已锚定无源网，无量级新物理。"
                    "对标公开传感链路预算量级。",
        features=[
            "复用 sensor_frontend 闭环（S1/S5/S7 死标量锚）",
            "MZI 干涉传感前端，光栅+Ybranch+SiN 全锚定基元组装",
            "生物/气体传感元件按参数化黑箱处理",
        ],
        applications=["即时诊断（POCT）生物传感", "片上气体/化学传感", "医疗即时检测微流控光路"],
        specs={"链路插损预算": "≤15dB（S1·仿真预期）", "判决锚": "sensor_frontend / S1·S5·S7",
               "复用基元": "GP-GRATING-EFF+GP-YBRANCH+GP-SIN-PL"},
        peers=[{"vendor": "公开 PICS 综述", "product": "干涉型光子传感前端",
                "specs": {"全链路插损": "≤15dB"}, "note": "公开综述量级（仿真预期·前瞻预研·未流片验证）",
                "source": "PICS / FBG 光纤传感链路预算综述"}],
        ci_status="",
    ),
    ShelfItem(
        id="IM-FTTR-PLC32",
        ...
        signal_ref="ITU-T G.671 / Telcordia GR-1209：1×32 PLC 每支路最大插损 ≤14.0 dB；"
                   "50G-PON（ITU-T G.hsp.50Gpon）公开路线",
        domain="photon",
        system_type="sensor_frontend",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"app": "fttr", "n_gratings": 0, "wg_length_cm": 0.0,
                     "n_ybranch": 5, "n_crossing": 0, "budget_db": 17.5, "tol_db": 1.5},
        design_note="FTTR 32 路 PLC = 5 级 Y-branch 级联（GP-YBRANCH，2^5=32 支路，含理想 5×3.01 dB 分光）"
                    "+ SiN 波导（GP-SIN-PL）；判决复用 sensor_frontend 自带死标量锚 S1/S5/S7。"
                    "复用已锚定无源网，无量级新物理。对标 ITU-T/G.671 公开规格。",
        features=[
            "复用 sensor_frontend 闭环（S1/S5/S7 死标量锚）",
            "1×32 PLC 5 级 Y-branch 级联，Ybranch+SiN 全锚定基元组装",
            "PON OLT/ONU 有源按黑名单处理",
        ],
        applications=["家庭/企业全光组网（FTTR）", "50G-PON 无源分光", "接入网无源光分路"],
        specs={"分路比": "1×32（5 级 Y-branch）", "每支路插损": "≤17.5dB（S1·对标 G.671·仿真预期）",
               "判决锚": "sensor_frontend / S1·S5·S7"},
        peers=[{"vendor": "ITU-T", "product": "1×32 PLC（G.671/GR-1209）",
                "specs": {"每支路插损": "≤14.0dB"}, "note": "公开标准量级（仿真预期·前瞻预研·未流片验证）",
                "source": "ITU-T G.671 / Telcordia GR-1209"}],
        ci_status="",
    ),
    ShelfItem(
        id="IM-TTD-5G",
        ...
        signal_ref="微波光子 TTD 公开综述：真时延网络片上光路损耗典型 ≤10 dB；"
                   "5G/6G 相控阵（国内外射频光子系统）公开路线",
        domain="photon",
        system_type="sensor_frontend",
        composition=["GP-GRATING-EFF", "GP-YBRANCH", "GP-SIN-PL"],
        default_req={"app": "ttd", "n_gratings": 2, "wg_length_cm": 1.0,
                     "n_ybranch": 0, "n_crossing": 2, "budget_db": 10.0, "tol_db": 2.0},
        design_note="TTD 前端 = 光栅耦合（GP-GRATING-EFF）+ 2×crossing 时延路由（GP-SIN-PL 延迟线）"
                    "+ SiN 波导（GP-SIN-PL）；电光调制按黑箱（负面清单）。"
                    "判决复用 sensor_frontend 自带死标量锚 S1/S5/S7；复用已锚定无源网，无量级新物理。"
                    "对标公开微波光子 TTD 链路量级。",
        features=[
            "复用 sensor_frontend 闭环（S1/S5/S7 死标量锚）",
            "真时延网络，光栅+2×crossing+SiN 全锚定基元组装",
            "电光调制器按黑名单处理",
        ],
        applications=["5G/6G 毫米波波束成形", "微波光子真时延", "相控阵光控时延网络"],
        specs={"链路插损预算": "≤10dB（S1·仿真预期）", "判决锚": "sensor_frontend / S1·S5·S7",
               "复用基元": "GP-GRATING-EFF+GP-SIN-PL+GP-YBRANCH"},
        peers=[{"vendor": "公开微波光子 TTD 综述", "product": "真时延网络片上光路",
                "specs": {"链路损耗": "≤10dB"}, "note": "公开综述量级（仿真预期·前瞻预研·未流片验证）",
                "source": "微波光子 TTD 公开综述"}],
        ci_status="",
    ),
    ShelfItem(
        id="IM-QKD-FULL-LINK",
        ...
        signal_ref="npj Quantum Information 3, e1700262 (2017) 公开论文：QKD Alice/Bob 芯片总插损 15/8 dB；"
                   "国内量子保密通信干线（京沪/武合）公开路线",
        domain="photon",
        system_type="qkd_link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"distance_km": 50.0, "target_distance_km": 50.0,
                     "published_floor_bps": 200.0, "fiber_loss_db_km": 0.2,
                     "alice_il_db": 15.0, "bob_il_db": 8.0, "detector_eff": 0.1,
                     "dark_count_prob": 1e-6, "misalignment": 0.015, "mu": 0.5,
                     "f_ec": 1.1, "rep_rate_hz": 1e9},
        design_note="QKD 干线收发前端 = 高效光栅耦合（GP-GRATING-EFF）+ SiN 波导引出（GP-SIN-PL）；"
                    "QKD 本质是信息论安全问题，按物理诚实映射到 qkd_link 系统类型（decoy-BB84 安全密钥率锚"
                    " S-QKD-SKR），不冒用量子比特读出锚。判决复用 decoy-BB84 渐近下界（Lo–Ma–Chen 2005）"
                    "已验证闭环，零新物理；含 Q-D67 护栏。对标公开 QKD 密钥率量级（等效验证）。",
        features=[
            "物理诚实映射：QKD 是密钥分发 → qkd_link 系统类型（decoy-BB84 安全密钥率信息论锚），非 qubit 读出锚",
            "Alice/Bob 收发前端，光栅+SiN 全锚定基元组装",
            "单光子源/探测器按黑名单处理",
        ],
        applications=["量子密钥分发（QKD）干线", "城域/骨干量子保密通信", "BB84 收发前端"],
        specs={"安全密钥率@50km": "≥200 bps（S-QKD-SKR·对标 IDQ Clavis 类公开量级）",
               "判决锚": "qkd_link / S-QKD-SKR",
               "复用基元": "GP-GRATING-EFF+GP-SIN-PL"},
        peers=[{"vendor": "公开 QKD 芯片论文", "product": "BB84 Alice/Bob 芯片",
                "specs": {"总插损": "15/8dB"}, "note": "公开论文量级（仿真预期·前瞻预研·未流片验证）",
    ...
    ShelfItem(
        ...
        signal_ref="IBM Heron R2 (2024, 156 qubit) 公开读数保真度 98.5%；本源悟空-180 读数 99.0%；"
                   "NISQ 典型单发读出 ≥97.5%（PostQuantum 2026 基准）",
        domain="hybrid",
        system_type="quantum_fidelity",
        composition=["GP-YBRANCH", "GP-SIN-PL"],
        default_req={"f01s": [round(4.6 + 0.2 * i, 2) for i in range(32)]},
        design_note="32-qubit 复用读出链 = Y-branch 路由（GP-YBRANCH）+ SiN 波导（GP-SIN-PL）读出网络；"
                    "判决复用 quantum_fidelity 已验证闭环（D-46×D-47）。复用已锚定无源网，无量级新物理。"
                    "对标公开超导量子系统读出保真度量级。",
        features=[
            "复用 quantum_fidelity 闭环（D-46 频率复用 + D-47 保真度）",
            "32-qubit 复用读出链，Ybranch+SiN 全锚定基元组装",
            "量子比特/读出腔按黑名单处理",
        ],
        applications=["超导量子计算读出/控制", "NISQ 规模扩展", "多量子比特复用读出链"],
        specs={"比特数": "32", "读出保真度": "≥97.5%（D-46×D-47·仿真预期·对标公开）",
               "判决锚": "quantum_fidelity / D-46·D-47"},
        peers=[{"vendor": "IBM", "product": "Heron R2 (156 qubit)",
                "specs": {"读出保真度": "98.5%"}, "note": "公开指标（仿真预期·前瞻预研·未流片验证）",
                "source": "IBM Quantum 公开披露 / 上海科技情报研究所对比表"}],
        ci_status="",
    ),
    ShelfItem(
        id="IM-OPA-2D",
        ...
        signal_ref="公开 OPA 硅光相控阵综述：片上 OPA 光路损耗典型 ≤12 dB；"
                   "固态雷达（国内外 OPA 路线）公开路线",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"p_tx_dbm": 6.0, "wg_length_cm": 1.0, "link_budget_db": 3.0, "n_channels": 4},
        design_note="OPA 2D 前端 = 高效光栅耦合（GP-GRATING-EFF）+ SiN 波导阵列（GP-SIN-PL）；"
                    "OPA 是光波束控器件，按物理诚实映射到 link 系统类型（IL 基），不冒用量子比特读出锚。"
                    "判决复用 link 系统预算锚死标量；复用已锚定无源网，无量级新物理。对标公开 OPA 光路量级。",
        features=[
            "物理诚实映射：OPA 是光波束控 → link 系统类型（IL 基），非 qubit 读出锚",
            "2D 相控阵前端，光栅+SiN 全锚定基元组装",
            "相位调制阵列按黑名单处理",
        ],
        applications=["固态激光雷达光束 steering", "光通信光束成形", "自由空间光互连波束控制"],
        specs={"光路损耗预算": "≤12dB（S1·对标公开·仿真预期）", "判决锚": "link / S1·S5·S7",
               "复用基元": "GP-GRATING-EFF+GP-SIN-PL"},
        peers=[{"vendor": "公开 OPA 综述", "product": "硅光 2D 相控阵",
                "specs": {"片上光路损耗": "≤12dB"}, "note": "公开综述量级（仿真预期·前瞻预研·未流片验证）",
                "source": "OPA 硅光相控阵公开综述"}],
        ci_status="",
    ),
    # —— M3+（货架 70→71）：IM-OPTCOMB-WDM 独立上架 ——
    #   此前与 IM-OPTCOMB（通用微梳前端）重合故未单独上架。本轮独立列出：
    #   专攻 WDM/ITU 网格锁定的多波长相干梳源，compositions 多纳入 MMI 复用，
    ...
    ShelfItem(
        ...
        signal_ref="Intel Market Research 光频梳 $58M(2025)→$108M(2034) CAGR 7.4%；芯片级微梳 CAGR 47.8%"
                   "（DWDM/时频同步/量子）；ITU-T G.694.1 DWDM 网格需求。",
        domain="photon",
        system_type="link",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-MMI-1X2"],
        default_req={"p_tx_dbm": 3.0, "wg_length_cm": 1.0, "link_budget_db": 3.0},
        honest_tier="前瞻预研",
        design_note="WDM 锁定微梳 = 高效光栅耦合 IO（GP-GRATING-EFF） + SiN 高 Q 微环梳（GP-SIN-PL，色散工程 "
                    "锁 ITU 网格） + MMI 分束复用（GP-MMI-1X2）；泵浦/探测/锁相环按黑箱（有源不物理级建模，"
                    "负面清单）。判决复用 link 系统预算锚死标量。与 IM-OPTCOMB（通用微梳前端）区分：本货架"
                    "专攻 WDM/ITU 网格锁定的多波长相干源，compositions 多纳入 MMI 复用，规格面向信道化 WDM。",
        ci_status="",
        features=[
            "锁定 ITU-T G.694.1 DWDM 网格的多波长相干梳源（与通用微梳前端区分）",
            "SiN 高 Q 微环梳 + 光栅 IO，复用已锚定无源网，无量级新物理",
            "面向 WDM-PON / 相干光通信 / 信道化射频光子学的梳状光源",
        ],
        applications=[
            "DWDM 多波长相干光通信（单梳替代多激光器阵列）",
            "WDM-PON 与数据中心内梳状光源",
            "信道化射频光子学与量子频率梳分发",
        ],
        specs={
            "波长网格": "ITU-T G.694.1 DWDM（100/200 GHz 可选）",
            "梳齿间隔": "≥100 GHz（相干 WDM 兼容）",
            "复用基元": "GP-GRATING-EFF + GP-SIN-PL + GP-MMI-1X2",
            "诚实等级": "前瞻预研（等效验证，非流片）",
        },
        peers=[
            {"vendor": "Intel / NTT", "product": "芯片级微梳（DWDM 锁定研究型）",
             "specs": {"网格": "ITU DWDM"}, "note": "对标公开研究型微梳量级，非本团队流片",
             "source": "Intel Market Research 光频梳报告 / NTT 微梳公开文献"},
        ],
    ),
    # —— v0.9.41（货架 71→75）：D 赛道 CPO 硅光 I/O 共封装新类上架 ——
    #   与既有 8 条 CPO 相关货架（IM-CPO-WDM5/WDM 级联、IM-CPO-OCS/光路交换、
    #   IM-CPO-16CH/通道数、IM-UCIE-OPTICAL/电学接口、IM-PHOTONIC-INTERPOSER/中介层、
    #   IM-CHIPLET-IO/die 间、IM-ONCHIP-NOC/片上网络、IM-OPTO-COMPUTE/光计算）**明确区分**：
    #   本批 4 条全部走新系统类型 cpo_optical_io，判据为「每通道插损 + **海岸线带宽密度**
    #   + 功率预算」，其中带宽密度是 CPO 的真实竞争维度（OIF/SE 公开：当前最先进 CPO
    #   ≈0.5 Tbps/mm vs AI chiplet UCIe ≈3 Tbps/mm，6× 差距）。
    #   严守红线：仅已锚定基元组装、判决纯死标量、LLM 不进判决、honest_tier=前瞻预研。
    ...
    ShelfItem(
        ...
        signal_ref="OIF-Co-Packaging-3.2T-Module-01.0（3.2T 引擎 = 32×CEI-112G-XSR 电 + 8×400G "
                   "FR4/DR4 光口）；Broadcom TH5-Bailly CPO 已部署；NVIDIA Quantum-X Photonics "
                   "Q3450-LD（144×800G InfiniBand，液冷）2026 出货；Spectrum-X Ethernet Photonics 2H2026 爬坡",
        domain="photon",
        system_type="cpo_optical_io",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-YBRANCH"],
        default_req={"couple_mode": "fau", "n_channels": 8, "lane_rate_gbps": 200.0,
                     "pitch_um": 250.0, "n_gratings": 2, "wg_length_cm": 1.0,
                     "n_ybranch": 1, "n_crossing": 1,
                     "p_tx_dbm": 0.0, "detector_sens_dbm": -20.0,
                     "il_budget_db": 12.0, "density_floor_gbps_mm": 100.0,
                     "min_margin_db": 3.0},
        honest_tier="前瞻预研",
        design_note="CPO 光 I/O = 光栅耦合器阵列 IO（GP-GRATING-EFF）+ SiN 低损波导扇出（GP-SIN-PL）"
                    "+ Y-branch 分光/监控支路（GP-YBRANCH）；判决走新系统类型 cpo_optical_io 三道死标量"
                    "锚：每通道插损 S-CPO-IL（GP-* 级联，D-67 能量守恒下界护栏）+ 海岸线带宽密度 "
                    "S-CPO-DENSITY（= 单通道速率/通道间距，P-CPO 间距几何下界护栏）+ 功率预算 S-CPO-BUDGET"
                    "（S1 同式）。与 IM-CPO-WDM5（WDM 级联 B4 锚）/IM-CPO-16CH（通道数扩展）区分。"
                    "EIC/驱动器/激光器按黑箱（有源不物理级建模，负面清单）。",
        ci_status="",
        features=[
            "以**海岸线带宽密度**为判据（CPO 真实竞争维度），非单一插损",
            "两道物理护栏对称夹逼：D-67 能量守恒下界防漏算 + P-CPO 间距几何下界（G.652 包层 125 µm）防虚报密度",
            "标准 250 µm 光纤阵列 pitch，工程可制造（非实验室极限值）",
        ],
        applications=[
            "51.2T/102.4T AI 以太网与 InfiniBand 交换机光引擎",
            "XPU/GPU 光 I/O（scale-up 光互连）",
            "可插拔向共封装演进的过渡评估（CPO vs LPO vs NPO 量化对比）",
        ],
        specs={
            "通道配置": "8×200 Gbps = 1.6 Tbps",
            "光纤阵列间距": "250 µm（标准 FAU）",
            "每通道插损": "≤12 dB（复现 10.63 dB·对标公开 6–12 dB 区间）",
            "海岸线带宽密度": "≥100 Gbps/mm（复现 800 Gbps/mm；物理上界 1600）",
            "判决锚": "cpo_optical_io / S-CPO-IL·S-CPO-DENSITY·S-CPO-BUDGET",
            "诚实等级": "前瞻预研（等效验证，非流片）",
        },
        peers=[
            {"vendor": "Broadcom", "product": "TH5-Bailly CPO（6.4T 光引擎）",
             "specs": {"形态": "CPO 交换芯片共封装"},
             "note": "公开已部署产品量级（非本团队流片，仅作规格标注与对标参照）",
             "source": "Broadcom 公开 CPO 路线 / 公开报道"},
            {"vendor": "NVIDIA", "product": "Quantum-X Photonics Q3450-LD",
             "specs": {"端口": "144×800G InfiniBand"},
             "note": "公开 2026 出货信息（非本团队流片）",
             "source": "NVIDIA 公开披露 / 公开报道"},
        ],
    ),
    ShelfItem(
        id="IM-CPO-OIO-16CH",
        ...
        signal_ref="OIF-Co-Packaging-3.2T-Module-01.0（3.2T CPO 引擎，8×400G 光口，~140 G/mm 海岸线）；"
                   "Semiconductor Engineering：缩小光纤阵列间距（FAU scaling）是提升 CPO 带宽密度的两条主路径"
                   "之一（另一为 WDM），「fiber pitches often exceed 100 microns」；"
                   "OIF EEI Panel OFC26：超大规模 scale-up 目标区 CPO 3 pJ/b @ 2.0 Tbps/mm",
        domain="photon",
        system_type="cpo_optical_io",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-YBRANCH"],
        default_req={"couple_mode": "fau", "n_channels": 16, "lane_rate_gbps": 200.0,
                     ...
                     "min_margin_db": 3.0},
        honest_tier="前瞻预研",
        design_note="把通道间距从 250 µm 压到 127 µm（标准光纤阵列细间距档）→ 总带宽 3.2 Tbps 而海岸线仅"
                    " 2.03 mm，密度复现 1574.80 Gbps/mm。⚠️ 该复现值**已达 P-CPO 物理上界 1600 的 98.4%**"
                    "（上界 = 200 Gbps / 125 µm G.652 包层直径）—— 再密就违反几何必然，护栏直接抛错拦截。"
                    "本货架兼作 P-CPO 护栏的贴身回归用例。与 IM-CPO-16CH（既有，通道数扩展口径）区分。",
        ci_status="",
        features=[
            "细间距 FAU（127 µm）逼近 ITU-T G.652 包层直径几何下界（125 µm）",
            "复现密度 1574.80 Gbps/mm = 物理上界 1600 的 98.4%，护栏贴身（判据非摆设）",
            "16 通道 3.2 Tbps 仅占 2.03 mm 海岸线",
        ],
        applications=[
            "3.2T/6.4T CPO 光引擎（OIF 3.2T IA 目标形态）",
            "超大规模 AI 集群 scale-up 光互连（OIF 目标 2.0 Tbps/mm）",
            "高密度 FAU 耦合方案可行性预评估",
        ],
        specs={
            "通道配置": "16×200 Gbps = 3.2 Tbps",
            "光纤阵列间距": "127 µm（细间距 FAU，物理下界 125 µm）",
            "海岸线宽度": "2.03 mm",
            "海岸线带宽密度": "≥100 Gbps/mm（复现 1574.80；物理上界 1600，占 98.4%）",
            "判决锚": "cpo_optical_io / S-CPO-IL·S-CPO-DENSITY·S-CPO-BUDGET",
            "诚实等级": "前瞻预研（等效验证，非流片）",
        },
        peers=[
            {"vendor": "OIF", "product": "Co-Packaging 3.2T Module IA",
             "specs": {"光口": "8×400G FR4/DR4", "海岸线": "~140 G/mm"},
             "note": "公开实施协议（非本团队流片，口径为模块级海岸线）",
             "source": "OIF-Co-Packaging-3.2T-Module-01.0"},
            {"vendor": "Semiconductor Engineering", "product": "CPO 带宽密度现状综述",
             "specs": {"当前最先进 CPO": "≈0.5 Tbps/mm", "AI chiplet": "≈3 Tbps/mm"},
             "note": "公开综述量级（非本团队流片）",
             "source": "semiengineering.com CPO 异构集成综述"},
        ],
    ),
    ShelfItem(
        ...
        target_app="chiplet 间光互连（die-to-die 光 I/O）、UCIe-Optical 光桥、光 CXL/内存池化、"
                   "AI 加速器分解式互连",
        signal_ref="OIF EEI Panel OFC26 官方表（oiforum.com）：ASIC UCIe-32G 标准封装 95 µm pitch "
                   "记 2.0 Tbps/mm，ASIC SerDes-200G 记 1.9–2.7 Tbps/mm；Semiconductor Engineering："
                   "AI chiplet（UCIe/OIF）边缘带宽密度 ≈3 Tbps/mm，与 CPO ≈0.5 Tbps/mm 存在 6× 差距；"
                   "OIF Compute Optics Interface（COI）面向 AI scale-up；Intel OCI 光计算互连 chiplet",
        domain="photon",
        system_type="cpo_optical_io",
        composition=["GP-GRATING-EFF", "GP-SIN-PL"],
        default_req={"couple_mode": "grating_array", "n_channels": 8,
                     "lane_rate_gbps": 32.0, "pitch_um": 45.0, "n_gratings": 2,
                     "wg_length_cm": 0.2, "n_ybranch": 0, "n_crossing": 0,
                     "p_tx_dbm": 0.0, "detector_sens_dbm": -20.0,
                     "il_budget_db": 10.0, "il_tol_db": 3.0,
                     "density_floor_gbps_mm": 300.0, "min_margin_db": 3.0},
        honest_tier="前瞻预研",
        design_note="couple_mode=grating_array（片间光栅阵列**直连，无光纤**）⇒ P-CPO 间距物理下界由"
                    " G.652 包层直径 125 µm 换为**模场直径 MFD@1550 = 10.3 µm**，故 45 µm 间距合法。"
                    "这验证 P-CPO 护栏**按耦合方式分档守下界**：一刀切用 125 µm 会误杀片间直连，"
                    "放松到无下界则可虚报密度。片间短距无分束/交叉 → 插损仅 2×光栅耦合 + 0.2 cm 波导。"
                    "与 IM-UCIE-OPTICAL（既有，电学接口标准口径）/IM-CHIPLET-IO（既有）区分。",
        ci_status="",
        features=[
            "按耦合方式分档的间距物理下界（无光纤直连 → MFD 10.3 µm 而非包层 125 µm）",
            "片间短距低损：仅 2×光栅耦合 + 0.2 cm 波导（复现 7.27 dB）",
            "面向 UCIe-Optical / OIF COI 的 die-to-die 光互连口径",
        ],
        applications=[
            "chiplet 间光互连（die-to-die optical I/O）",
            "UCIe-Optical 光桥与封装内光网络",
            "光 CXL / 内存池化（分解式 AI 互连）",
        ],
        specs={
            "通道配置": "8×32 Gbps（UCIe-32G 类）",
            "耦合方式": "光栅阵列直连（无光纤）",
            "通道间距": "45 µm（物理下界 = MFD@1550 10.3 µm）",
            "海岸线带宽密度": "≥300 Gbps/mm（复现 711.11；物理上界 3106.8）",
            "判决锚": "cpo_optical_io / S-CPO-IL·S-CPO-DENSITY·S-CPO-BUDGET",
            "诚实等级": "前瞻预研（等效验证，非流片）",
        },
        peers=[
            {"vendor": "OIF", "product": "EEI Panel OFC26 · UCIe-32G 标准封装",
             "specs": {"pitch": "95 µm", "边缘带宽密度": "2.0 Tbps/mm"},
             "note": "⚠️ 公开值为 **2D 凸点阵列口径**，本货架 711 Gbps/mm 为**单排 1D 海岸线口径**，"
                     "两者不可直接比较（golden 已按 1D 折算取值）",
             "source": "oiforum.com OIF_EEI_Panel_OFC26.pdf"},
            {"vendor": "Intel", "product": "OCI（Optical Compute Interconnect）chiplet",
             "specs": {"形态": "光计算互连 chiplet"},
             "note": "公开产品动向（非本团队流片）",
             "source": "Intel 公开披露 / 公开报道"},
        ],
    ),
    ShelfItem(
        id="IM-CPO-ELS-FIBER",
        ...
        target_app="CPO 可维护性方案（前面板可更换光源）、OIF 3.2T 光引擎、NPO/CPO 混合演进、"
                   "规避「光器件故障导致整交换机报废」的运维痛点",
        signal_ref="OIF-Co-Packaging-3.2T-Module-01.0（3.2T = 32×CEI-112G-XSR + 8×400G FR4/DR4 光口）；"
                   "Broadcom 第三代 102.4T Davisson（TH6）CPO 交换机引入**前面板可现场更换激光源**，"
                   "规避 CPO 维护痛点；锐捷 25.6T 硅光 NPO、Ragile RA-BC6932 引入 ELSFP 接口；"
                   "CPO 单端口功耗 5.5–7W（800G），较可插拔降低 50–70%（Yole/OIF 公开汇总）",
        domain="photon",
        system_type="cpo_optical_io",
        composition=["GP-GRATING-EFF", "GP-SIN-PL", "GP-YBRANCH", "GP-CROSSING"],
        default_req={"couple_mode": "fau", "n_channels": 8, "lane_rate_gbps": 400.0,
                     "pitch_um": 250.0, "n_gratings": 2, "wg_length_cm": 1.0,
                     "n_ybranch": 1, "n_crossing": 1,
                     "p_tx_dbm": 0.0, "detector_sens_dbm": -20.0,
                     "il_budget_db": 12.0, "density_floor_gbps_mm": 100.0,
                     "min_margin_db": 3.0},
        honest_tier="前瞻预研",
        design_note="可维护性导向：把温度敏感的激光器外置为可现场更换模块（ELS/ELSFP），光经光纤送入"
                    "CPO 引擎 ⇒ 光栅耦合 IO（GP-GRATING-EFF）+ SiN 波导（GP-SIN-PL）+ crossing 交叉"
                    "（GP-CROSSING，ELS 光路与本地环路交汇）+ Y-branch 分光。8×400G = 3.2 Tbps 正对"
                    "OIF 3.2T IA 光口配置，密度复现 1600 Gbps/mm（= 200G 档物理上界的 2 倍，因单通道"
                    "速率翻倍，上界同步升至 3200）。与 IM-LASER-INT（片上异质集成光源黑箱）区分。",
        ci_status="",
        features=[
            "外置可更换激光源（ELS）：规避 CPO「光器件故障 = 整机报废」运维痛点",
            "8×400G = 3.2 Tbps，正对 OIF 3.2T IA 光口配置",
            "密度随单通道速率线性提升（物理上界同步，非虚报）",
        ],
        applications=[
            "OIF 3.2T CPO 光引擎（标准形态）",
            "NPO/CPO 混合演进与可维护性方案评估",
            "AI 交换机前面板可更换光源（ELSFP）前端",
        ],
        specs={
            "通道配置": "8×400 Gbps = 3.2 Tbps（OIF 3.2T IA 光口形态）",
            "光源方案": "外置可更换激光源（ELS / ELSFP）",
            "每通道插损": "≤12 dB（复现 10.63 dB·对标公开 6–12 dB 区间）",
            "海岸线带宽密度": "≥100 Gbps/mm（复现 1600；物理上界 3200）",
            "判决锚": "cpo_optical_io / S-CPO-IL·S-CPO-DENSITY·S-CPO-BUDGET",
            "诚实等级": "前瞻预研（等效验证，非流片）",
        },
        peers=[
            {"vendor": "Broadcom", "product": "Davisson（TH6）102.4T CPO 交换机",
             "specs": {"光源": "前面板可现场更换激光源", "功耗": "1.6T 下降低 70%"},
             "note": "公开产品动向（非本团队流片）",
             "source": "Broadcom 公开披露 / 公开报道"},
            {"vendor": "OIF", "product": "Co-Packaging 3.2T Module IA",
             "specs": {"光口": "8×400G FR4/DR4", "电接口": "32×CEI-112G-XSR"},
             "note": "公开实施协议（非本团队流片）",
             "source": "OIF-Co-Packaging-3.2T-Module-01.0"},
        ],
    ),
