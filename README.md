...
> 当前版本：**v0.5**（2026-08-23 · 系统级里程碑：M7 收口（可调 WDM + 量子门/纠错拓扑 + 大规模基准）+ 护城河两件（L0 IR 开放标准 + 验证合约工业化）+ Track A 逆设计纵深（谱形目标 → 形状 → 混合 → 多波长 → 3D adjoint → 3D 截面 → 3D 端口联合验收 → 谱形×3D）+ 四十八面板）
