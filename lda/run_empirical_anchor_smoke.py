...
覆盖：
  ① harness 实证锚题解析（BENCHMARK_DEFS 35 = B1-B27 + E1-E7 + S1；E 题 golden 来自实测语料；
...
def main():
    ...
    check("BENCHMARK_DEFS 35 题（B1-B27+E1-E7+S1）", len(BENCHMARK_DEFS) == 35
    ...
    check("参考候选 35/35 PASS（双 ground）", npass == len(specs) == 35,
          f"{npass}/{len(specs)}")
