...
class CountConsistencySmoke(unittest.TestCase):
    ...
    def test_benchmark_b30_e9_s13_split(self):
        ...
        # v0.9.99 B-19 加 B313-B328 → 323；v0.9.100 B-20 加 B329-B344 → 339；v0.9.104 B-22 加 B361-B373 → 373；v0.9.105 B-23 加 B374-B386 → 381；v0.9.106 B-24 加 B387-B399 → 394。
        self.assertEqual(len(b_ids), 394, f"B 题应 394（B1-B399，缺口 B35/B38/B39/B69/B72 预留），实际 {len(b_ids)}")
        ...
        self.assertEqual(max(b_ids, key=lambda x: int(x[1:])),
                         "B399", f"B 题最大编号应 B399，实际 {max(b_ids, key=lambda x: int(x[1:]))}")
    ...
    def test_readme_benchmark_counts(self):
        ...
        self.assertIn("B1-B399", self.readme)
