...
## 当前账本：CI 机器断言守护（动态，FAIL=0 即绿）· **CI core 83 条**
...
- 诚实边界：当前属**原理验证级非流片级**；实证语料 30 条中 **25 条 A 级可公开溯源**（含 DOI / 公开 URL 定位符，第三方可独立复验），**5 条 B 级（仅有描述性来源、无定位符）禁止作 golden 进判决**；7 道实证锚题中 **6 道 golden 为真·可溯源实测**（E1 因无公开可溯源 n_eff 实测源、且标量 FDFD 对高对比度 SOI 不达标，标为 B 级待全矢量求解器，不计入可溯源计数）。
- ⚠️ **判决路径独立性（D-64 如实披露）**：golden 真实只是必要条件，candidate 还必须**独立求解**。当前 7 道实证锚中**仅 E2 接通独立求解器**（FDFD 1.959 ↔ 实测 1.892，|diff|=0.067），**其余 6 道（E1、E3-E7）candidate 仍为占位自证（candidate≡golden，|diff|≡0）** —— 其 PASS 只能算「自洽」，**不算「验证」**，已在 note / 报告 candidate_desc / CI smoke 断言三处如实标注（宁可红，不可假绿）。复制 E2 模式到其余六道是下一阶段主线；真实晶圆厂 NDA 实测仍属发动期。
