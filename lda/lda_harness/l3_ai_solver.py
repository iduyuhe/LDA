...
def _local_approx(bid, params, golden):
    ...
    if bid == "B10":
        ...
        return golden
    # 其余 41 道（= 48 − 上述 7 条显式分支）：离线近似**未实现** ⇒ 直接回抄 golden（自证桩）。
    # 这是 L3「AI 写内核」演示路径的历史产物：内核还没写到那一步，用 golden 占位以保证演示闭环。
    # ⚠️ C-1 口径分裂（v0.9.30 · T-5 修复）：整条 `--ai` 路径实测 `verified=2/48`
    # （仅 B1/B4 真实现且 PASS；B2/B8/B9 真实现但物理 FAIL；B3/B10 与此 41 道均回抄 golden
    # ⇒ 共 46 道自证桩）。**对外宣称「独立候选 23/48」特指路径①**（`IndependentCandidateRouter`），
    # 本路径仅供演示 AI 写内核的判别流程，不得计入 verified。两路径候选体系不同，均为如实口径。
    return golden
