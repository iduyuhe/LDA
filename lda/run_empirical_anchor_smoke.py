...
覆盖：
  ① harness 实证锚题解析（BENCHMARK_DEFS 41 = B1-B27 + E1-E7 + S1-S7；E 题 golden 来自实测语料；
...
def main():
    ...
    check("BENCHMARK_DEFS 41 题（B1-B27+E1-E7+S1-S7）", len(BENCHMARK_DEFS) == 41
    ...
    check("参考候选 41/41 PASS（双 ground）", npass == len(specs) == 41,
          f"{npass}/{len(specs)}")
