...
3. **未审计 WebUI 前端 JS 质量**（`index.html` 368 KB / `store.html` 75 KB 内联脚本未做 lint）。
   ✅ **已补盲（2026-09-21 · T4.2）**：抽取 **8 段内联脚本共 5034 行** ⇒ eslint 9.39.4（隔离安装 · 不进主仓库依赖）**0 error / 0 fatal**；质量信号 `no-unused-vars` **84** · `no-empty` **3**；**判据有效性已验证**（注入 `no-undef` / `no-redeclare` / 语法错 / `no-cond-assign` 四类真缺陷**全部被报出**）。报告 §5。⚠️ **边界**：仅静态 lint，未做运行时行为审计。
