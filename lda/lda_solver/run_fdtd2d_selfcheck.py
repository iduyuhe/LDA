"""LDA · L3 自研 2D FDTD 核 · 物理定律锚（TMM 解析解 / 柱面波）交叉校验闭环。

机器优先：跑多个解析可判定的干净结构，2D FDTD 谱 vs TMM 谱断言一致；
再加"点源柱面波 |Ez|·√r 常数"作为真·二维判别（验证 2D 核心 + 四向海绵）。

与 1D selfcheck 同哲学：判据用"物理特征是否复现"，不苛求逐点绝对吻合。
关键：A/B/C/D 为分层膜（y 方向平移不变 → 退化为一维），由 tmm.py 这一
非 AI 物理定律 ORACLE 校验；E 为点源激发的 2D 柱面波，校验真二维传播。
"""
...
from lda.lda_solver.fdtd2d import solve_spectrum, run_greens_test
...
def _report(name, layers, wls, tol):
    ...
    print(f"  {'wl(um)':>8} {'FDTD2D_T':>10} {'TMM_T':>9} {'delta':>8}")
...
def main():
    print(">> LDA 自研 2D FDTD 求解（C 级自主，机器优先接口）交叉校验")
    print(">> ORACLE = TMM 多层膜解析解（一维退化极限）+ 点源柱面波（真二维）")
    ...
    # 用例 E：点源柱面波（真·二维 + 四向海绵无回反射）
    ok_g = _report_greens("E. 点源柱面波 |Ez|·√r 常数（真二维）", 0.20)
    all_ok = all_ok and ok_g

    print("\n" + "=" * 48)
    print(">> 总判定：", "PASS — 2D 自研核通过物理定律锚校验" if all_ok
          else "FAIL — 2D 自研核未通过校验，需排查")
