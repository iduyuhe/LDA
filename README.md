...
> 
> 当前版本：**v0.9.20**（2026-09-02 · **B14 定向耦合器接线 + golden 语义修正 · 严格独立 14 → 15**：接线段再下一城，且抓出 **D-66「怀疑 golden 本身」第 4 例**——B14 原式 λ/(2Δn)=15.5µm 是**完全转移长度**（P₂=sin²(π/2)=1.0，RK4 实证）被错标 3dB 点，真 3dB 点=λ/(4Δn)=**7.75µm**（P₂=0.5）；golden 15.5→7.75、tol 0.5→0.25 同比重定、同源消费点 `_dc_supermode_core`（Δβ·L=π→π/2）等 4 处一并修正、kernel_seal smoke 动态跟随实测 5/5；候选=新写 `lda_solver/dc_cmt_solver.py` 数值传播+FFT 拍频谱峰（B3/B4/B20 同款频域周期提取方法学）；🔴 方法学发现：二模恒耦合系统传播矩阵是精确旋转，「数值传播+根查找」路线全部退化为机器精度（8e-15~4.6e-13）撞 1e-12 自证桩判据 ⇒ 必须 FFT 谱峰路线（baseline 1.56e-4 非机器精度）；严格独立 14→15、自证桩 33→32、降级仍 1（E2）；判据窗口 1.56e-4 < tol 0.25（1560×）< 反向信号 5.71（22.9×）；全量回归 85 全绿确认 golden 修正零下游破坏；⚠️ 诚实边界：独立 15/48、自证桩 32）·
