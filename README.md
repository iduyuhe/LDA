...
> 
> 当前版本：**v0.9.21**（2026-09-02 · **B1 米氏散射接线 · 严格独立 15 → 16**：新写 `lda_solver/mie_solver.py` 完整 Mie 级数（B&H 4.53 维度形式 + Wiscombe 截断，纯 numpy 递推零外部依赖）↔ golden Rayleigh 一阶极限——物理同源（麦克斯韦）、方法独立（一阶展开 vs 全阶求和），|diff|=Rayleigh 固有截断误差随 x 单调（-0.001%@x=0.01 → 1.388%@x=0.4）；数值自检双锚（x→0 收敛 Rayleigh + 递推 vs scipy 球贝塞尔 ≤3e-8）；🔴 **环境确定性修复**：golden 的 `use_miepython=True` 原会在装有 miepython 的环境自动切完整 Mie ⇒ golden 环境相关、判决不可复现——default_params 钉死 False，Mie ORACLE 保留给显式外部验货；判据窗口 3.945e-5 < tol 2e-4（5.1×）< min 信号 1.246e-3（6.2×），tol 未动；严格独立 15→16、自证桩 32→31、降级仍 1（E2）；⚠️ 诚实边界：独立 16/48、自证桩 31）·
> **v0.9.17**（2026-09-02 · **量子侧五道接线 · 严格独立 7 → 12**：延续 P0 接线段，把 B12/B13/B22/B23/B24 五道早宣称有严格侧、harness 从未真接的锚接成真独立候选；严格独立 7→12、自证桩 40→35、降级仍 1（E2）；tol 仅 B13 收紧 50×/B24 按实测重定、余未动；全量回归抓出 B24 候选返回 numpy 标量致 run_harness JSON 序列化 TypeError，已修（candidate 返回值 float 包裹）；⚠️ 诚实边界：独立 12/48、自证桩 35）·
> **v0.9.16**（2026-09-02 · **P0-3 闭合 + 光子侧低成本批次接线 · 严格独立候选 4 → 7**：①**P0-3 缺口闭合** —— `fdfd_ng` 正式登记进 `BENCHMARK_CANDIDATES`，并在 `harness.py` 把三分类常量 `CANDIDATE_CLASS_{STRICT,DEGRADED,STUB}` 定为**全库唯一定义处**，`candidate_class(bid)` **判序固定为「先判降级 → 再查登记表 → 否则自证桩」**（序若写反，E2 会被判独立且 PASS ⇒ `verified` 从 7 虚报成 8 —— 反向自检 A 已精确复现这一假绿）；路径②同步消费三分类 ⇒ E2 在两条路径**都是降级量级参考**，对外自证桩口径从「降级+自证」的 44 回归真实值 40。②**光子侧 B3/B4/B20 接线** —— 三道 FSR 族锚统一改用**数值响应谱的频域峰周期**拟合（谐振/干涉峰满足 光程 = m·λ ⇒ **1/λ 严格等距**；教科书闭式 FSR_λ = λ²/光程 只是该等距性在 λ0 处的**一阶连续化**，与「相邻峰实测波长间距」相差 O(1/m)：B3 6.7% / B4 0.59% / B20 1.29% —— 故候选统一按频域周期取值，否则会**假红**）；网格 `n_grid=50001` 为刻意标定（三道残差 1.7e-8 / 1.9e-8 / 4.7e-10，离护栏判据 1e-12 有 ≥467× 余量、同时远低于各自 tol），**峰位只做三点抛物线细化、不做牛顿/二分精化** —— 打磨到机器精度会让 |diff| 掉到 1e-12 以下、与自证桩按值不可区分。**三道 tol 一律未放宽**（放宽 tol 等于取消验证，是 P0 纪律红线），实测余量 B20 达 2000×。③**护栏升级为三分类双向复核**：非自证桩 ⇒ |diff| 必须**非零**（回落 golden 即穿帮）；自证桩 ⇒ |diff| 必须**为零**（新接线被误分类吞掉即穿帮）—— 只做前半条会放过「路由改坏、标签仍真」的假绿，也会放过漏算。**五处反向自检全部会响**（判序反转 / B20 静默回落 / B1 漏分类 / 对外口径回退 / CLI 自身断言），篡改手法与实测报错写入 CHANGELOG。⚠️ **诚实边界（未变）**：严格独立 **7/48** · 降级量级参考 1 · 自证桩 **40** —— 提高的是「可被外部验货的比例」而非单道验证强度；剩余 40 道按 P0 计划继续接线）·
> **v0.9.15**（2026-09-02 · **P0-2 独立性接到对外验货面 · 让"可被外部验货"真正兑现**：v0.9.14 接通的 4 道独立候选**只在路径①（内部 smoke）成立**，对外验货面走②③、仍显示 diff=0。本次三线收口：②**CLI 默认路径接线**——`harness.py` 新增 `_SpecShim`+`IndependentCandidateRouter`（按 spec_id 路由到已登记独立候选，未登记者诚实回落参考候选），`run_harness.py` 默认候选改走该路由 ⇒ **对外主报告 `verified` 0 → 4**，报告头部按题分列「N 项独立 / M 项自证」避免「48/48 通过」被读成「48 项已验证」；`report.py` 新增 `verified_count()` 作**全库唯一权威口径**，断言升级为**双向护栏**（多算=假绿 / 少算=倒退 / verified+stub≡total）③**AI 候选路径治理**——订正 B3/B10 两处**注释与实现不符**（注释写"正确实现"、实际 `return golden`），新增 `_LOCAL_INDEPENDENT_IDS`（真有独立实现的仅 B1/B2/B4/B8/B9）与 `is_independent()`（**LLM 启用时一律 False**——红线：LLM 不进判决路径），实测 `--ai` 的 verified **假绿 45 → 诚实 2**（B2/B8/B9 故意错判 FAIL，仅 B1/B4 通过）④**对外账本去硬编码**——`/api/verification_ledger` 的 `judgment_paths` 原写死 `independent_candidate=["E2"]`，**而 E2 恰是 v0.9.14 已降级那道、新接的 4 道一道都没出现** ⇒ 对外验货面失真，与 ci_core 计数漂移属同一类缺陷（写死 vs 实际）；改为从 `BENCHMARK_DEFS.candidate`×`BENCHMARK_CANDIDATES` **动态推导三分类**，生产实测 **严格独立 4 / 降级量级参考 1 / 自证桩 43（三类和=48 校验通过）**。⑤**新增第 ⑦⑧ 两项常驻护栏**钉死口径不漂移（可证伪性 smoke 6→8 项）：⑦「端点口径 ≡ 冒烟口径」⑧「路径②（CLI 对外主报告）verified 口径 ≡ 路径①，且 48 题全部按题标注」——后者是因为 `ci.yml` 直跑 `run_harness.py` 而**本地 `--tag core` 不跑它**，存在覆盖盲区。🔴 **护栏自身全部做了反向自检，且抓出一个真教训**：⑧ 的**首版无效**——临时把 `router.__call__` 改成全回落 golden，它**仍报 PASS**，因为 `independent` 标签来自 `is_independent`（查登记表）、被改坏的却是 `__call__`（执行层）⇒ **标签 ≠ 行为**；加强为**按值复核**（标独立的题 |cand−golden| 必须非零）后才真正会响（精确报 `假独立=['B25','B26','B27','B9']`）。**顺带查出 CLI 自身断言有同一个洞**并补同样防线，且按路径语义收敛：路径②路由器承诺「方法学不同源」⇒ |diff| 必非零；路径③ `--ai` 验证的是「AI 内核算得对不对」⇒ |diff|≡0 表示**算对了**（B1/B4 即此情形），一刀切会误伤（实测确实误报过）。⚠️ **诚实边界（未变）**：**独立候选仍只有 4/48**，本次只是让对外如实显示这个数字、不再宣称 45 项已验证；剩余 43 道自证桩按 P0 计划继续接线）· **v0.9.14**（2026-09-02 · **P0-1 锚题独立候选化 · 反自证桩第一刀**：2026-09-02 战略审计实测 **48 锚中 47 道为自证桩**（candidate≡golden、|diff|≡0 恒 PASS），且 B 类 28 道**连接入独立候选的入口都没有** → 首开 B 类接入口（`BENCHMARK_CANDIDATES` 注册表 + `candidate` 字段分发），接通 **B9/B25/B26/B27 四道真独立候选**（解析闭式 golden ↔ 严格数值对角化 candidate：电荷基 41 维 eigh / L=6 多能级+Fock 162 维 eigh，实测 rel 0.22% / 1.98% / 2.02%，均非 0 ⇒ 真可证伪）；tol 按实测偏差 2.2~2.5 倍余量放宽并逐条写明依据；新增 **`run_benchmark_falsifiability_smoke.py` 常驻护栏**——①独立候选数下限 ②正向 PASS ③**反向测试：10% 参数扰动必 FAIL（证明 tol 未放水）** ④灵敏度登记（最小可检出扰动 B9 2%、B25/B26/B27 5%）⑤全量无回归，入 CI core **84→85**；`candidate_status` 字段把"降级量级参考"（E2）从散文 note 变为**机器可读**，杜绝虚报独立强度）· **v0.9.13**（2026-09-01 · **R16 实测证伪+诚实边界 C 降级** + CI 达标线政策化（90%+）** + D-67 回归修复 · 链路预算漏算 3.0103dB 分光 + 双护栏**：v0.9.10（D-66）判定「3.01dB 是 1×2 功率均分的几何必然、非器件品质指标」**本身正确**，但实现时把 `engine_ybranch_split` 的默认输出 `value` 从「含分光的分支插损」改成了「过量损耗」，而该 `value` **同时是链路预算的被加数量**（`_photon_cascade_il` 的 `n_yb * yb`）→ **每个分束器少算 3.0103 dB**。受影响 6 条：GC-PLC-1X8 **0.33→9.3309 dB**（-9.0）、GC-PLC-1X16 **0.44→12.4412**（-12.0）、GC-SENSE **7.63→13.6508**（-6.0）、GC-QKD-TX **7.54→13.5638**（-6.0）、GC-CPO-8CH **7.62→10.6335**（-3.0）、GP-YBRANCH **0.10→3.1103**（拿过量损耗比总插损 golden，语义错配）。**为何 84/84 全绿没抓到（三重失真叠加）**：①插损 metric 方向 `le`（越小越 PASS）→「少算损耗」被伪装成「设计变好」；②`run_golden_product_smoke` 只校验 PASS **条数**不校验数值；③`evaluate` 在 metric 名对不上时**静默回退**到 `value`。**修复（分离而非替换，两个量都真实存在）**：引擎同时输出 `value`/`metric`=`split_loss_dB`（链路预算量=3.0103+过量，向后兼容）+ `excess_loss_dB`（同名字段显式暴露，供实证锚对照）；新增常量 `SPLIT_LOSS_3DB=−10·log₁₀0.5`；`resolve_corpus_engine` 改按 metric 名取值；`_loss_verify`/`_loss_cheap` 新增 `field` 参数，`YbranchLoss` 显式传 `excess_loss_dB` 使「目标/搜索/判决」同量纲。**新增两道护栏（均做反向测试证明会响）**：①**能量守恒下界**——每个 1×2 分束的每支路插损不可能低于 3.0103dB（能量守恒，与工艺无关）；⚠️ 必须**按贡献项逐项守底**，用「总插损≥n_yb×3.0103」的混合判据会让 GC-CPO-8CH/SENSE/QKD-TX 因其他损耗垫高而**逃逸（反向测试只抓住 2/5）**，逐项守底才 5/5；②**metric 语义错配硬失败**——禁止静默回退到 `value`（宁可红不可假绿）。**护栏的护栏**：`run_golden_product_smoke` 新增 D-67 反向测试，临时注入坏引擎断言两道护栏都命中，否则 FAIL——**没被验证过的护栏不算护栏**。🔴 **工程铁律（新）**：**改引擎默认输出 `value` 的语义前必须 grep 全部 `["value"]` 消费点**（不只改断言）；**「越小越 PASS」的方向性 metric 必须配物理下界护栏**）· **v0.9.10**（2026-09-01 · **实证锚逐字核实（D-66）· 语料库 A 级 100%**：5 条 B 级语料（E-SOI-NEFF-220 / E-SIN-NEFF-300 / E-YBRANCH-LOSS / E-RING-FSR / E-GRATING-EFF）经**逐字核实**原文后全部升级——**语料 30 条 A 级 30/30（100%），B 级清零；可溯源实证锚题 6/7 → 7/7**。核实中抓出三类问题：①`E-SOI-NEFF-220` 的 n_eff=2.63 **是错值**（文献与 3 个独立模式求解器一致给出 2.44~2.46；2.63 实为 λ≈1.39µm 处取值，偏离为其自称 ±0.02 的近 10 倍）→ 改判 **n_g=4.18**（arXiv:2011.03273 racetrack 实测 FSR=8.6nm 反演，自洽校验 8.59nm ✅）；②`E-SIN-NEFF-300` 无可溯源实测出处 → 改判 **n_g=2.2834**（DOI 10.3390/coatings10040309，几何按文献照实改写保证 golden 同源）；③`E-YBRANCH-LOSS` 原 3.4dB **量纲不符**（含 3.01dB 理想分光，那是 1×2 均分的几何必然、非被测量）→ 改判实测**过量损耗 0.28±0.02dB**（DOI 10.1364/OE.21.001310）。另 `E-RING-FSR` 原 9.15nm 系**解析反算值**→换实测 8.6nm；`E-GRATING-EFF` 原 0.45 无出处→换实测 0.42（DOI 10.1063/1.3304791，**结构不同源已披露**）。排除 1 处差点误用的来源：arXiv:1909.09538 的 −3.05dB 前文为 `"This simulation"` → **仿真非实测**。配套：`EmpiricalMeasurement` 新增 `note` 字段（溯源核实批注，判定路径不读）；A 级达标线 80%→**100%**；下游同步 10 文件约 24 处。🔴 **顺带修掉 GitHub 主干红灯**：`empirical_bank.traceability()` 相对导入在以脚本方式直跑时抛 ImportError，而该脚本在 ci.yml 却**不在本地 CORE_SMOKES** → **主干自 v0.9.8 起一直红、本地全绿**；已修双路导入并纳入 core 门禁（**CI core 83→84**））· **v0.9.9**（2026-09-01 · **判决路径独立性整改（D-64）· 首道真·实证交叉验证**：审计发现实证锚**判决路径为空**——`_harness_reference_candidate` 直接返回 golden，致 E1-E7 七道 `|candidate − golden| ≡ 0.0`，恒 PASS、零验证价值（golden 真实只是必要条件，**candidate 必须独立求解**）。本次以 E2 为样板打通：几何对齐实测器件（500nm→**1000nm** 宽）+ golden 换成 A 级实测群折射率 **n_g=1.892**（Munoz et al., Sensors 17, 2088，OFDR 环腔实测 + MZI 交叉验证）+ 新增独立求解器 `_fdfd_ng_candidate`（标量亥姆霍兹 FDFD 本征模算 n_eff(λ) → 固定网格中心差分得 n_g），得 **FDFD 1.959 vs 实测 1.892，|diff|=0.067 ≤ tol 0.10** —— LDA 首道「实测↔独立求解」真交叉验证。余 6 道保留占位但**在 note / candidate_desc / smoke 断言三处如实标注为自证桩**（宁可红不可假绿）；实证锚 smoke 新增 2 条 D-64 断言（23→25）。同批量化求解器能力边界：标量 FDFD 仅适**低对比度**波导（SiN 2.0/1.44 可用），**不适高对比度 SOI**（3.71~3.78 vs 参考 4.19，差 10% 且 n_eff 网格未收敛）→ E1 保留 B 级待全矢量求解器。语料 29→**30 条**，A 级 24→**25（83.3%）**，**可溯源实证锚题 5/7 → 6/7**；🔍 **D-65 精度边界（同批实测发现）**：FDFD 候选在该器件上网格过粗（0.3µm 芯厚仅约 4.6 格），n_g 随计算窗口散射 **±0.04** → E2 据此**只判「量级一致」、不宣称「精度验证」**，并新增窗口鲁棒性断言（5 个计算窗口全部落在容差内，证明 PASS 非凑巧）与 **R16** 登记；**对外验货面同步整改**——`run_harness.py` 报告与 `/api/verification_ledger` 现显式标注「自证闭环 ≠ 验证结论」（含 JSON `self_consistent/verified` 机器可读字段 + CI 断言防丢失），账本 `ci_core` 改动态取值（此前写死 82 而实为 83））· **v0.9.8**（2026-09-01 · **实证锚来源边界与溯源审计（D-63）**：确立「仅限公开论文 / datasheet / 公开测量数据集 + 必须可公开溯源」来源边界；新增 `lda_harness/provenance.py` 三级**机器可判**溯源分级（A=含 DOI/arXiv/公开 URL 可作 golden 进判决，B=无定位符**禁止**作 golden）与独立审计器 `run_provenance_audit.py`（已入 CI core，82→83 条）；补 6 条 A 级真实实测语料（Sridaran & Bhave, Opt. Express 18(4) 3850 (2010)）；审计暴露并整改 E3「解析公式冒充实测锚」（golden 9.15→实测 10.44，形成实测↔解析交叉验证）；E1/E2 因 n_eff 无公开可溯源实测源改标 B 级待溯源——**可溯源实证锚题 5/7**）· **v0.9.7**（2026-09-01 · **生产安全加固 · POST 重计算端点登录闸门**：在 v0.9.6 四重并发护栏基础上，于 `_heavy_guard` 统一入口追加登录闸门——store 会话令牌或管理员 Bearer 令牌任一放行，未登录直接 401 且不消耗缓存/并发资源；GET 验货端点（cpo_array / verification_ledger）仍无鉴权，维持「可被外部验货」战略可达性）· **v0.9.6**（2026-09-01 · **生产安全加固 · POST 重计算端点统一并发护栏**：50 个仿真/设计类 POST 端点（`/api/ring_fdtd`、`/api/sparams`、`/api/adjoint_design` 等）补「每端点独立串行锁 + 全局并发上限（按核数封顶 4）+ 参数哈希缓存 + 入参体积硬上限」四重护栏，堵死「同时打 50 个端点」的总并发敞口；轻端点（鉴权/商店/生态）不进护栏 · **v0.9.5**（2026-08-31 · **生产安全加固 · 验货端点并发护栏补全**：`GET /api/benchmark_crosscheck`（无鉴权公开 GET、默认实跑 `run_crosscheck(quick=True)` ~9s）补同款三护栏（串行锁+缓存），至此所有「公开 GET + 默认实跑重计算」端点均带护栏 · **v0.9.4**（2026-08-31 · **生产安全加固 · CPO 验货端点并发护栏**：`GET /api/cpo_array` 加三道护栏——输入硬上限（oe≤48/ch≤96/lane≤16，超则 400 防 OOM）/ 全局串行锁（任意时刻至多一个重计算，并发 429 防 ThreadingHTTPServer 下并行打爆）/ 默认配置结果缓存（TTL 120s，重复 curl 秒回）· **v0.9.3**（2026-08-31 · **验证可信度外部验货 · 全量验证账本端点**：新增 `GET /api/verification_ledger`（无鉴权、可 curl 验货）——暴露全部验证资产分类与计数（physical-law 38 道 / ORACLE 依赖 3 道 / empirical 7 道 = 48 题）+ CI core 82 条 + CPO 规模死锚 100,096/250,240 ACCEPT，诚实标注 R2/R3/R4 开放缺口）· **v0.9.2**（2026-08-31 · **阶段2 · CPO 共封装光引擎阵列（十万级真实器件样例）**：新增 `lda_harness/cpo_array.py` 层次化生成器（阵列 n_oe → 光引擎 n_ch → 波长通道 → 波长 lane）+ `run_cpo_array_demo.py` 全链闭环——**100,096 器件 / 2,176 条独立光路 / 97,920 布线网 + 4,352 外部 IO**，不再是「N 个 Waveguide 串成一条链」而是真实 CPO 器件构成（微环调制器 MRM / WDM add-drop 解复用环 / 功率监测抽头 / 光栅耦合器 / 互连波导段）；参数由物理反解而非拍脑袋——微环半径 R = m·λ/(2π·n_eff)（m=91 **整数**谐振级数、n_eff=2.45 → 7.530–7.713 µm），光栅周期 Λ = λ_c/(n_eff,gr − sin θ) = 0.612 µm（θ=15°，齿宽 0.367 / 齿隙 0.245 µm 满足 DRC 双约束）；几何采用**端口线对齐 + 零跳线**（行内连接全为水平段、无回折、无跨行垂落 → 同层短路数为 0 由几何保证，而非靠 LVS 兜底）；实测 **DRC 100,096/100,096 全过 · LVS ACCEPT（0 违规 · 97,920/97,920 网表全匹配）· 注入断路反例 REJECT · GDS 38.98 MB / 359,040 元素 4.02s · 芯片 13.14 × 7.89 mm = 103.67 mm² · 全链 8.34s**；`run_cpo_array_smoke.py` 21 条断言入 CI core（80→81）· **v0.9.2 规模纵深**：新增 `run_cpo_array_scale_smoke.py`——CPO 阵列推到 **250,240 器件（2.5× 默认 100,096）** 实跑全链 + 近线性预算守卫（25 万全链 ≤60s），防生成器回归 O(n²)，入 CI core（81→82）。诚实边界：仅建模无源光子层（有源器件按黑箱·负面清单）、工艺为公开文献近似非真实 PDK、只做版图闭环未做光学仿真、未流片无实测回流）· **v0.9.1**（2026-08-30 · **S13 设计良率锚 DFY（锚缺口分析 P0 · 对标 EDA yield 能力）**：新增 `S13` 设计良率锚——环形 FSR 在光刻容差（环周长 σ=±1% 高斯）下命中 ±2% 规格窗口的概率，**解析闭式（高斯容差精确积分，保留 1/L 非线性，非一阶近似）↔ 蒙特卡洛采样（N=20000 固定种子）双算法互证**，解析 0.954413 vs MC 0.954750、偏差 0.034pp ≤ tol 1pp；同一物理定律两种独立算法互证 = 非 AI ground，载体为 B4 环形 FSR 定律、零新物理。良率随容差单调下降（σ=0.5%→99.99% / 1%→95.44% / 2%→68.28% / 4%→38.30%），可直接回答客户「容差收紧到多少良率能到 99%」（实测：须收紧到 0.5% 以内）。纯标准库零依赖；`run_statistical_anchor_smoke` 扩展 11 条 S13 断言（34 PASS / 0 FAIL，含 AST 查 import 的零 LLM 红线断言）；题库 46→47 题· **量子侧回归入 CI（P0-1 · 解除 R1）**：9 个量子 smoke 此前全部不在 core 门禁（双引擎一半裸奔），实测全部 PASS 后纳入——7 个亚秒级（ir_quantum/quantum_devices/quantum_design/multiqubit/multiqubit_fidelity/readout_fidelity/readout_chain）+ 2 个含 FDTD 仿真约 180-200s（splitter_readout D-63 / splitter_readout_cal D-66，配 400s 超时覆盖防慢机误判）；CI core 70→80 条**· **v0.9.0**（2026-08-29 · **商业化里程碑（货架商品化三批 + 商务闭环全线收口）**：① 货架商品化 58/58——首批 10 + 剩余 48 全量填充（特点/用途/关键规格/国际对标四字段），国际对标全部带可溯源来源（IEEE 802.3 系列 / ITU-T G.671·G.9804·G.9807.1 / CWDM4 MSA / NVIDIA·TSMC·Broadcom 公开路线 / 期刊文献 / 市场报告），诚实标注「仿真预期·未流片」② 站内同类对比（货架卡勾选 2~4 → 对比表 7 维度含对标来源）+ 评价/提意见需求收集（管理员聚合含联系方式）③ 定制需求导流（6 方向 chips + 该方向开放货架实时推荐「先看现成」+ 提交成功二次导流）④ 商务闭环加固：登录注册安全审计 8 项（账号+IP 双维度限流 / 反枚举 / 密码上限 / 令牌 TTL）/ 管理员重置密码闭环（临时密码+强制改密）/ 全站一键登录（nav.js 全局弹窗）/ 对公申请转账凭证截图上传 ⑤ 战略审计文档 docs/lda_strategy_audit_2026-08-29.md；CI core 69→70（run_store_flow_smoke 商务闭环入 core）**）· **v0.8.55**（2026-08-29 · **管理后台上线打通商务闭环：新增 admin.html（令牌登录·待处理申请列表·一键审批生成兑换码·复制/下载链接），修正申请响应空头邮件承诺为准确文案；生产部署真实对公收款账户（上海农商银行陈行支行 32434508010036375）+ 联系人/电话，并加固管理员令牌（LDA_ADMIN_TOKEN 环境变量）；货架/开放数维持 58/50；量子 8 维持咨询制；CI core 维持 69 条**）· **v0.8.54**（2026-08-29 · **对公收款程序适配：创新超市 insights.html 增加「对公购买」申请入口 + 顶部「对公收款说明」弹窗，后端新增 /api/purchase/request、/api/admin/purchase_requests、/api/admin/purchase/{id}/approve 端点，对公转账到账后管理员审批生成绑定货架的兑换码；货架/开放数维持 58/50；量子 8 维持咨询制；CI core 维持 69 条**）· **v0.8.52**（2026-08-28 · **持续扩货架：新增 5 个光子缺口品类开放下载（微环谐振滤波、1×N 功分树、微波光子真延时、波导气体传感、光栅耦合阵列），货架 48→53、开放 40→45；量子 8 维持咨询制；docs/store_launch/04_market_analysis.md 新增趋势段；CI core 维持 69 条**）· **v0.8.51**（2026-08-28 · **持续扩货架：新增 5 个光子缺口品类开放下载（OPA 固态 LiDAR、相干接收 90°混频、片上光网络 NoC、多芯光纤扇出、光纤陀螺 Sagnac），货架 43→48、开放 35→40；量子 8 维持咨询制；docs/store_launch/04_market_analysis.md 新增趋势段；CI core 维持 69 条**）· **v0.8.50**（2026-08-28 · **持续扩货架：新增 5 个光子缺口品类开放下载（MZM 调制器、PSR 偏振分束、光子中介层/CPO、光计算 ONN、OCT 成像），货架 38→43、开放 30→35；量子 8 维持咨询制；docs/store_launch/04_market_analysis.md 新增趋势段；CI core 维持 69 条**）· **v0.8.49**（2026-08-28 · **持续扩货架：新增 5 个光子缺口品类开放下载（相干 ZR、微环调制器、XGS-PON、WSS、VOA），货架 33→38、开放 25→30；量子 8 维持咨询制；docs/store_launch/04_market_analysis.md 新增趋势段；CI core 维持 69 条**）· **v0.8.48**（2026-08-28 · **持续扩货架：新增 8 个光子缺口品类开放下载（FR4 200G/400G 每通道、400G DR4、100G LR4、50G-PON、可重构光开关、FMCW 接收、环形生物传感），货架 25→33、开放 17→25；量子 8 维持咨询制；docs/store_launch/04_market_analysis.md 新增趋势段；CI core 维持 69 条**）· **v0.8.47**（2026-08-28 · **扩货架：开放 17 个光子主流货架下载（新增 1.6T DR8 + 800G DR8/PSM4/FR4/CWDM4/LPO-112G/WDM 8CH/DWDM 40CH/CPO WDM5·OCS/FTTH PLC 8-16/传感 RING·MZI·LIDAR/光 chiplet IO/激光集成），量子 8 个列咨询制（出口管制合规红线）；货架 24→25；新增 docs/store_launch/04_market_analysis.md；/api/shelf 加 open 字段前端区分「下载/咨询制」**）· **v0.8.46**（2026-08-28 · **工厂+商店化：货架→设计就绪包（GDS+网表+DRC+死锚报告），开放 3 个试点货架下载（IM-PSM4 / CWDM4 / FR4），新增 /api/shelf/{id}/package 与 /download 端点 + 兑换码授权，前端货架卡片加「下载设计包」按钮（免费看、付费下）**）· **v0.8.45**（2026-08-28 · **展示能力扩张：GC 对标 24 + 创新货架 24 + 能力演示场景 12 项（真实模块/端点画廊 + 核心引擎在线自检）**）· **v0.8.44**（2026-08-28 · **B 技术纵深三连：并行布线 / 规模锚升 4k / 相关簇锚**：②并行布线  route_batch（workers=1 串行逐位一致，>1 进程池零依赖并行；收益边界实测：链式 12µs 无收益、复杂版图 400ms/网 workers=4 达 2.67×；拥塞感知有顺序依赖并行下明确拒绝不伪并行；run_parallel_routing_smoke 5/5 入 CI core 68→69）· S11 规模锚默认 1000→4000（预算 5s→10s，实测 4k 全链 0.07s 余量 140×；smoke 纵深升 8k 近线性守护）· S12 增相关簇锚（连续 ≥3 通道同向偏离 = 系统级簇漂移 → REJECT；盲区实证：旧三锚 ACCEPT、簇锚唯一捕获；插损/保真度已配置不误伤；统计锚 smoke 19→23）· harness 46/46 全过；CI core 69 条· **v0.8.43**（2026-08-28 · **WebUI 能力展示层：GC 对照 / 创新超市 / 规模现场实测**：新增 4 个 API——（20 条 GC 对标， 现场 25/25）、 + （20 条货架，单条真跑 design_pipeline 死标量判决，LLM 不进路径）、（1k/4k/32k 全链实测，32k 亚秒活演示）；新增  深色展示页（GC 对照表 + 货架评估卡片 + 规模柱状图），index 顶部入口；静态页白名单防路径穿越；路由冒烟 63 实跑 + 57 静态全绿；CI core 维持 68 条· **v0.8.42**（2026-08-28 · **S2 纵深三连：拥塞估计 + 算力实测 + 阵列统计锚**：②布线  CongestionMap 拥塞图（A* 启发式叠加惩罚，默认 None 逐位一致）——平行网绕同一障碍最大占用 4→2、拥挤格 222→13；run_astar_route_smoke 10/10。④算力实测结论：细粒度几何内核（4 元素 tuple）numba njit 反而更慢（1.69s vs 0.44s，装箱开销>编译收益）→ 锁定纯 Python 为最优，不引入无效依赖。⑤统计锚  新增 **S12 阵列分布锚**（均值锚+下界锚+离群锚 AND 判决，抓单点锚盲区「均值好看但某通道崩坏」），题库 45→46；run_statistical_anchor_smoke 扩展守护。CI core 维持 68 条· **v0.8.41**（2026-08-28 · **几何 DRC 拔钉子：间距检查 O(n²) → 网格候选**： 最小间距检查从纯 O(n²) 双重循环改为均匀网格候选（bbox 重叠⟺共格精确等价，与 LVS 同法），判决逐位一致（violations/checked/spacing_note 对拍零差异）；8k 多边形 0.08s 近线性、高密度 4000 全重叠 0.44s（旧分钟级）；run_drc 系 smoke 全绿，CI core 维持 68 条· **v0.8.40**（2026-08-28 · **构建器拔钉子：port_abs 索引缓存**：profile 定位 build_chain_case 布线生成的 O(n·m) 存量低效（`port_abs` 每次线性扫全组件，构建 87% 时间耗此）→ `lda_layout/placement.py` 组件查找走索引缓存（O(1) 查表，`_port_abs_cache_clear` 兜底），公共签名与坐标语义零变化；**32k 器件全链（构建+放置+布线+LVS）0.98s**（旧外推 ~960s，~1000×），缩放斜率稳定 1.7–2.7× 近线性；run_scale_smoke 新增 3 条「4k 规模纵深」检查 16/16 PASS，CI core 维持 68 条· **v0.8.39**（2026-08-28 · **LVS 拔钉子：O(n²) → 空间索引**：`lda_l2/lvs.py` 四处复杂度热点根治（端口锚点网格 3×3 邻域 / port_abs 索引化 / 交叉检测路径 bbox 网格候选），纯标准库零新依赖；判决一致性铁证——3 case×4 规模+单层逐字节 diff 零差异、随机 400 折线 62138 交叉对零漏报零误报；LVS 1k 0.88s→0.01s、32k 仅 0.58s（旧外推 ~960s，~1600×），缩放斜率 O(n²)→近线性；run_lvs_smoke 27 PASS / scale 13 / chip_scale 8 全绿，CI core 维持 68 条· **v0.8.38**（2026-08-28 · **GC 库扩至 20 项 + 创新超市货架扩至 20（市场信号驱动）**：市场调研收集可溯源公开规格（IEEE 802.3bs / ITU-T G.671 / 商用 datasheet / 公开文献实测），GC 库 4→20（新增 16 = 光互连 3 + 无源网络 3 + 光交换·传感·LiDAR 3 + 量子通信 3 + 量子计算 4，含祖冲之三号 99.18% / IBM Heron R2 98.5% / Google Willow 99.33% 三家量子对标）；货架 5→20（新增 15 = 光子 12 + 量子 3，800G DR8 / 40ch DWDM / FTTH PLC / OCS 直连 / FMCW LiDAR / QKD 收发 / XPU 光 IO / 三家量级读出链）；`run_golden_product_smoke` 现 25/25（5 GP + 20 GC），`run_innovation_market_smoke` 现 20/20×3 守护全 PASS；修复 v0.8.37 GOLDEN_IDS 对 ChipBenchmark 崩溃 bug；CI core 维持 68 条（数据扩展非新增 smoke）· **v0.8.37**（2026-08-27 · **GC 库扩至 4 项（整芯片级对标续扩）**：`golden_product_benchmarks` 增 GC-SENSE（光子传感前端整芯片，MZI 全链路 IL 13.6dB≤15）/ GC-QCTRL-COMM（商用量子控制芯片 6-qubit 读出保真度 99.78%≥99%），复用 GP-* dB 级联 / D-46×D-47 已验证闭环，零新物理；`run_golden_product_smoke` 现 9/9（5 GP+4 GC），CI core 维持 68 条（数据扩展非新增 smoke）；另出正式讨论稿 `LDA_封装测试闭环与数字孪生协同_讨论稿.md`· **v0.8.36**（2026-08-27 · **整芯片级对标（GC-*）**：`golden_product_benchmarks` 从器件级 GP-* 扩到整芯片级 GC-*——新增 `ChipBenchmark` 类，光子走 GP-* 基元 dB 级联（S1 同构）、量子走 `design_multiqubit_fidelity`（S4 同构），死标量比对；首批 2 个 GC（GC-CPO-8CH 商用 CPO 8 通道光引擎每通道 IL 10.6dB≤15、GC-QCTRL 量子读出保真度 99.78%≥97%）、golden 来自公开产品规格（IBM Research / 本源悟空-180 公开披露）可溯源；`run_golden_product_smoke` 现覆盖 7/7（5 GP+2 GC），CI core 维持 68 条（数据扩展非新增 smoke）· **v0.8.35**（2026-08-27 · **创新超市货架库扩展（2→5 货架）**：在 v0.8.34 货架注册表上新增 3 个前瞻预研货架——IM-SENSE-RING（微环传感前端，复用 link/S1-S7）/ IM-LASER-INT（片上激光源集成，异质集成黑箱源·负面清单）/ IM-QCOM-LINK（5 比特量子频率复用读出，复用 D-46×D-47）；仍严守"组合已锚定基元（GP-*）"护栏，active 器件按黑箱源处理、不新增未锚定基元；CI core 维持 68 条（货架为数据扩展，非新增 smoke 文件）· **v0.8.34**（2026-08-27 · **创新超市（前瞻预研货架）**：新增 `lda/lda_l2/innovation_market.py` 货架注册表——`ShelfItem` 组合已锚定基元（GP-*）+ 公开信号驱动的前瞻预研预设计；`run_innovation_market_smoke` 红线下护栏（composition 全锚定 / 结构可行 / honest_tier=前瞻预研）入 CI core（67→68）；`docs/innovation_market.md` 为可浏览目录（B 素材）· v0.8.33（2026-08-27 · **系统类型注册表（提案编译器系统级纵深）**：`proposal_compiler` 增 `SYSTEM_TYPES`（link 默认 + wdm_demux + quantum_fidelity），`design_pipeline` 增 `system_type` 参数向后兼容；wdm/quantum 复用 `design_wdm_advanced` / `design_multiqubit_fidelity` 已验证闭环（B4 / D-46×D-47 死标量判决），零新物理；`run_system_types_smoke` 入 CI core（66→67））· **v0.8.31**（2026-08-27 · **版图几何级 RC 寄生估算（设计侧主权闭环收口）**：新增 `lda/lda_l2/parasitic_rc.py` 几何级 R/C 寄生估算（主权 RC 表，非 foundry 工艺级 deck）；接入 `tapeout_pipeline` S3.5（提供 GDS 实跑、无版图诚实 SKIP）；`run_parasitic_rc_smoke` 入 CI core· **v0.8.32**（2026-08-27 · **产品级基准对照库（实证锚产品级扩展 + B 生态播种，免流片）**：新增 `lda/lda_l2/golden_product_benchmarks.py`——对标已公开验证（实测/厂商 datasheet/开源 PDK 表征）的器件性能死标量，LDA 引擎规格驱动再设计+复现 5/5 PASS；`run_golden_product_smoke` 入 CI core（65→66）；`docs/golden_product_benchmarks_report.md` 为 B 生态播种硬核素材）· 历史：v0.8.30（2026-08-27 · **CLI 深化 + gdsfactory 兼容 + 计数守护固化**：`lda gf <gdsfactory_component.py>` 把 gdsfactory 组件转 LDA 链路 spec（B 级可选依赖、未装优雅降级）；`lda check --gds <file.gds>` 导入任意 GDSII 跑**主权几何 DRC 快查**（gds_drc，子集诚实标注非 foundry 全量）；`run_count_consistency_smoke` **加固**（只扫顶行防历史链误匹配、版本线=pyproject 动态校验，此前静默失效的 61≠62 漂移已根治）；新增 gds_export.parse_gds_polygons / gdsfactory_bridge。· 注：v0.8.29 开发者 CLI 钩子（design/check/report 三命令薄壳 + run_cli_smoke 入 CI core）、v0.8.28 UI 双修（目标误差列 bug + 统计卡片死卡）、v0.8.27 千器件演示、v0.8.26 规模扩展、v0.8.24 多层版图、v0.8.24 LVS 签核、v0.8.10 首轮持续维护均已落地（同见下方历史链）。彼时账本：**22 引擎（光子 15 + 量子 7）+ 11 包 = 33 类端到端 · 46 题（B1-B27 + E1-E7 + S1-S12）· CI core 70 条**。· 历史：v0.8.28（2026-08-27 · **UI 双修**：①目标误差列 0.0000 bug——`design_engine` 的 `rec["err"]` 用 cheap 估算（Koch 反解数学精确 → err 恒 0），改验证后按真实 metric 误差重算（Transmon 候选现显示 0.0047/0.0086/0.0137，best 按真实误差选优 E_C=0.15）②统计卡片死卡——c-harness/c-ai 前端从未赋值，后端 /api/status 补 harness_passed/harness_total/ai_candidates 字段 + 前端 renderStatus 填值，标签统一）· 历史：v0.8.27（2026-08-27 · **千器件芯片级演示**：千器件版图接入芯片级演示闭环——1000 器件链式链路 + 2D 放置 + 多层布线 → GDS 导出（IO 光栅接入）+ DRC 1000/1000 + LVS ACCEPT 双闸，全链路 0.99s，`run_chip_scale_demo.py` 8/8 入 CI core）· 历史：v0.8.26（2026-08-27 · **千器件规模扩展（版图差距 #7 收官）**：`lda_harness/scale_anchor.py` 千器件链式链路全链路 **0.92s 完成 ACCEPT**——跨行跳线走 M2 层；LVS 相交检测 **bbox 预检 3.6× 提速**；harness **S11 规模锚**（题库 **44→45**）；版图 7 差距**全部闭合**）· 历史：（2026-08-27 · **多层版图（版图差距 #6 落地）**：`lda_l2/layers.py` 层栈定义 + **多层 LVS**（`run_lvs_multilayer` 层感知几何恢复 + via 桥接 + 跨层垂直投影重叠安全=介质隔离）+ harness **S10 锚**（题库 **43→44**））· 历史：v0.8.24 LVS 签核深化（版图差距 #5）：`lda_l2/lvs.py` 版图-原理图一致性检查（签核级）——版图网表**从布线几何独立恢复** → 六类违规死标量检出 → ACCEPT/REJECT 确定性判决 + harness **S9 锚**（题库 **42→43 题**）+ 芯片级签核双闸 + WebUI `/api/link_design` 返回 lvs_report）· 历史：v0.8.23 第二梯队-2（多端网 Steiner + 2D 放置 + 有源基元三件套）· v0.8.22 第二梯队-1（A* 全局最优布线）· v0.8.10 首轮持续维护（内核纵深五击 + 芯片级验收闭环 + 器件库主流封口 + 流片级验证管道 + 计数一致性门禁：22 引擎 + 11 包 = 33 类端到端 + **芯片级四锚验收** + 仿真级芯片设计闭环演示 + **流片级验证管道**（PDK→DRC→SS/TT/FF 工艺角→LVS→实测回流）+ harness 题库 **45 题**（B1-B27 + E1-E7 + S1-S11）+ Phase 3 统计锚 + **Phase 4 提案编译器** + CI core 61 条）
...
## 当前账本：CI 机器断言守护（动态，FAIL=0 即绿）· **CI core 85 条**

- **22 引擎 + 11 包 = 33 类端到端（光子 15 + 量子 7）**
- **48 题（B1-B28 物理定律锚 + E1-E7 实证锚 + S1-S13 系统锚）**
- **创新超市：58 货架（光子 50 开放下载 + 量子 8 咨询制·出口管制合规红线）**
- 主权纪律：A 级永不借（商业 EDA/商业 NDA-PDK）；B 级借今踢后（gdsfactory/Meep/KLayout/SAX fork 主权副本，可选）；C 级第一天自主（L0 IR/L1 协议/L3 求解核/物理定律锚）。
- 诚实边界：当前属**原理验证级非流片级**；实证语料 **30 条全部 A 级可公开溯源**（含 DOI / arXiv / 公开 URL 定位符，第三方可独立复验），**B 级已清零（D-66 逐字核实后 5 条全部升级）**；7 道实证锚题 **7 道 golden 均为真·可溯源实测**（可溯源率 6/7 → **7/7**）。
- 🔍 **D-66 逐字核实（2026-09-01）**：5 条 B 级语料逐字核对原文后 —— ①`E-SOI-NEFF-220` 的 n_eff=2.63 **经核实为错值**（真值 2.44~2.46；2.63 系 λ≈1.39µm 处取值，偏离为其自称 ±0.02 的近 10 倍）→ 改判 **n_g=4.18**（arXiv:2011.03273 racetrack 实测 FSR=8.6nm 反演）；②`E-SIN-NEFF-300` 无可溯源实测出处 → 改判 **n_g=2.2834**（DOI 10.3390/coatings10040309，几何按文献照实改写保证 golden 同源）；③`E-YBRANCH-LOSS` 原 3.4dB 含 3.01dB 理想分光（**几何必然、非被测量**）→ 改判实测**过量损耗 0.28±0.02dB**（DOI 10.1364/OE.21.001310）；④`E-RING-FSR` 原 9.15nm 系解析反算值 → 换文献实测 **8.6nm**；⑤`E-GRATING-EFF` 原 0.45 无出处 → 换实测 **0.42**（DOI 10.1063/1.3304791，**结构不同源已披露**：文献为全刻蚀光子晶体孔阵）。另排除 1 处**差点误用**的来源（arXiv:1909.09538 的 −3.05dB 前文为 "This simulation" → 仿真非实测）。
- ⚠️ **判决路径独立性（D-64 如实披露）**：golden 真实只是必要条件，candidate 还必须**独立求解**。当前 7 道实证锚中**仅 E2 接通独立求解器**（FDFD 1.959 ↔ 实测 1.892，|diff|=0.067），**其余 6 道（E1、E3-E7）candidate 仍为占位自证（candidate≡golden，|diff|≡0）** —— 其 PASS 只能算「自洽」，**不算「验证」**，已在 note / 报告 candidate_desc / CI smoke 断言三处如实标注（宁可红，不可假绿）。注意：**E1 的 golden 已升 A 级，但不改变其判决路径仍为自证桩这一事实**（且标量 FDFD 对高对比度 SOI 不达标：直波导 n_g=3.74 对实测 4.18 差约 10%）。🔴 **R16（亚网格 ε 平均）已于 2026-09-01 实测证伪**：上 averaging + 网格细化 dl=24→64 均无效，根因=①FDFD 标量求解器精度不足（直波导 n_eff 偏差 0.18~0.37）②**对象不对齐**（golden 来自弯曲/环器件，FDFD 解直波导，弯曲使 n_g 天然高 ~0.46）。故 E1 **保持自证桩、不强行接 FDFD 对照**，FDFD 直波导候选仅作量级参考（**C 方案诚实边界降级**，与 D-66 一致）。真实晶圆厂 NDA 实测仍属发动期。
- ⚠️ **候选求解器的精度边界（D-65 如实披露，2026-09-01 修正）**：FDFD 本征模求解器对高反差细波导**精度不足**——直波导 n_eff SOI=2.62（文献~2.44，+0.18）、SiN=1.61（文献~1.98，−0.37），n_g 散射 **±0.04~0.08**。🔴 **原 D-65 诊断「网格过粗」不准确**：实测 dl=24→64（λ/64）n_g 变化 <0.02，早已收敛，非网格截断误差；±0.04~0.08 散射实为**计算窗口尺寸扫描**造成（clad 改变波导约束）。两层真因：①FDFD 标量求解器精度不足 ②**对象不对齐**（golden 来自弯曲/环器件，FDFD 解直波导，弯曲使 n_g 高 ~0.46）。🔴 **R16（亚网格 ε 平均）已证伪**：averaging + 细网格均不能解锁 E1/E2/E3。因此 **E2 只判「量级一致 + 判决路径真实」，不宣称「精度验证」**——其 0.10 容差中大部分是数值不确定度而非物理裕度；判定本身鲁棒（5 个计算窗口全落容差，已钉进 smoke）。**R16 降级为诚实边界 C**：FDFD 直波导候选与环 golden 几何不同源、精度不足，仅作量级参考（与 D-66 一致）。
- ✅ **锚题独立候选化 P0-1（v0.9.14 · 2026-09-02 · 战略审计 R1 第一刀）**：2026-09-02 战略审计实测发现 **48 锚中 47 道是自证桩**——`build_harness_specs` 对所有非实证锚一律落 `_harness_reference_candidate`（candidate≡golden、|diff|≡0 恒 PASS）；且 **B 类 28 道连"接入独立候选"的入口都没有**（只有 E 类能指定 `candidate: "fdfd_ng"`）。**全绿 ≠ 可证伪**。本次首开 B 类接入口（`BENCHMARK_CANDIDATES` 注册表 + `candidate` 字段分发），并接通 4 道真独立候选：**B9 / B25**（golden=Koch 色散解析 ↔ candidate=电荷基严格对角化，41 维 eigh）、**B26**（golden=Blais 微扰闭式 ↔ candidate=L=6 多能级+Fock 联合对角化，162 维 eigh）、**B27**（由严格 χ 反推 t_CZ）。实测偏差 **B9/B25 rel 0.22%、B26 rel 1.98%、B27 rel 2.02%，均非 0 ⇒ 真可证伪**。tol 相应放宽（B25 1e-6→0.05、B26→1e-4、B27→30ns），依据=实测偏差的 2.2~2.5 倍余量，逐条写入 note。新增 **`run_benchmark_falsifiability_smoke.py` 常驻护栏**（入 CI core，84→85）：①独立候选数下限 ②正向 PASS ③**反向测试：10% 参数扰动必 FAIL（证明 tol 没放水到"什么都抓不住"）** ④最小可检出扰动 ≤10%（实测 B9 2%、B25/B26/B27 各 5%）⑤全量 48 锚无回归。⚠️ **诚实边界**：**B26 在 g 扰动 +1% 处 diff 反而变小（1.68e-6 < 未扰动 4.57e-5）**——扰动方向与近似误差**偶然抵消**，属物理正常现象，但意味着**小幅系统误差存在检测盲点**，这正是放宽容差所付的代价，故反向测试取 10% 稳健档而非单点小扰动；另 **B27 与 B26 共用同一数值 χ**，独立性弱于 B26，只验证「χ→t_CZ 换算链路」，不重复计入独立强度（已用 `candidate_status` 机器可读字段区分"严格独立"与"降级量级参考"，E2 即后者）。**剩余自证桩按 P0 计划继续接线** —— 🔄 **v0.9.16 已降至 40/48**（B3/B4/B20 三道新接线转严格独立，见下条；当前自证桩 = 全部 S 类 13 道 + E1/E3-E7 + B1/B2/B5-B8/B10-B19/B21-B24/B28）。⚠️ **作用域澄清**：LDA 有**三条验证路径各自用不同 candidate**——①`build_harness_specs`+`cand_map`（本轮改到，**4 道独立**）②`harness.run(specs, ReferenceCandidate)`（`run_harness.py` 对外主报告，**仍全自证、`verified=0`**）③`L3AISolverCandidate`→`_local_approx`（MCP/L1/WebUI，41 道 `return golden`）。**「4 道可证伪」当时仅在路径①成立**，对外验货面走②③、仍显示 diff=0。✅ **该缺口已由 v0.9.15（P0-2）闭合**：②`run_harness.py` 默认候选改为 `IndependentCandidateRouter`（按 spec_id 路由，未登记者诚实回落），对外主报告 `verified` **0 → 4**；③`l3_ai_solver.py` 订正注释与实现不符、新增 `is_independent`（**LLM 启用时一律 False**），`--ai` 的 verified **假绿 45 → 诚实 2**；对外账本 `/api/verification_ledger` 三分类改为动态推导并新增第 ⑦ 项常驻护栏钉死「端点口径 ≡ 冒烟口径」。
- ✅ **P0-2 独立性接到对外验货面（v0.9.15 · 2026-09-02）**：`verified` 从「只在内部 smoke 里存在」变成对外账本上的真实数字。**三分类定稿：严格独立 4 道（B9/B25/B26/B27）· 降级量级参考 1 道（E2）· 自证桩 43 道（三类和 = 48 校验通过）**。四处改动：①`harness.py` 加 `_SpecShim`+`IndependentCandidateRouter`（把 dict spec 适配成候选所需对象接口 ⇒ 路径①候选零改动复用到路径②），`BenchmarkResult` 加 `independent` **三态**字段（True=独立 / False=自证 / None=未标注旧路径 ⇒ 渐进式改造、旧路径行为完全不变）②`report.py` 新增 `verified_count()` 作**全库唯一权威口径**，混合态下报告分列「N 项独立 / M 项自证」，不再让「48/48 通过」被读成「48 项已验证」③`run_harness.py` 断言升级为**双向护栏**（多算=把自证桩当已验证 / 少算=独立候选被降级 / verified+stub≡total 不得有第三态漏算）④对外账本去硬编码并加第 ⑦ 项护栏。⚠️ **护栏自身已做反向自检**：临时改坏端点分类条件后 smoke 立刻 exit=1 且精确报「E2 从降级掉到自证」，确认会响后才恢复代码——**没被验证过的护栏不算护栏**。⚠️ **诚实边界未变**：独立候选仍只有 **4/48**，本次只是让对外如实显示，而非提高验证强度。⚠️ **生产实测又查出 1 处对外与实际的口径差（已修，根因留 P0-3）**：端点 `harness_cli` 原写 `stub=43`，而 CLI 报告**实际是 44**——因为 `fdfd_ng` **未登记进** `BENCHMARK_CANDIDATES`，E2 在**路径①**是真 FDFD 候选（实测 golden 1.892 / cand 1.9587 / |diff|=0.0667，分类=降级量级参考），在**路径②**却回落成自证桩 ⇒ 路径② 的「非独立」= 43+1 = 44。已改照实写 44 并新增 `trichotomy_totals` 把两套口径同时暴露，护栏 ⑦ 补两条断言（反向自检：改回 43 立刻报 `CLI stub=43≠44`）。🔴 **登记 P0-3**：修复需把 `fdfd_ng` 登记进表**且**让 `is_independent` 尊重 `degraded_ordinal`，否则 E2 会被判独立且 PASS ⇒ `verified` 从 4 虚报成 5（假绿）——这是「接线越多越容易假绿」的典型；当前属低估不属虚报。**✅ 已于 v0.9.16 闭合（见下条）。**
- ✅ **P0-3 闭合 + 光子侧低成本批次（v0.9.16 · 2026-09-02 · 严格独立 4 → 7）**：**三分类刷新：严格独立 7 道（B3/B4/B9/B20/B25/B26/B27）· 降级量级参考 1 道（E2）· 自证桩 40 道（三类和 = 48 校验通过）**。①**P0-3**：`fdfd_ng` 登记进 `BENCHMARK_CANDIDATES`；`harness.py` 新增三分类常量（全库唯一定义处）与 `candidate_class(bid)` / `describe_trichotomy()`，`is_independent` 改为「== STRICT」。🔴 **判序是关键**：必须**先判 `candidate_status == degraded_ordinal` 再查登记表** —— 反过写，E2 一登记就会被判成 strict ⇒ `verified` 从 7 虚报 8。反向自检 A 精确复现该假绿（`独立数=8≠路径①7；路径②三分类=(8,0,40)≠路径①(7,1,40)`）。②**光子侧 B3/B4/B20**：新增 3 个频域峰周期拟合候选（`fp_fsr_peakfit` / `ring_fsr_peakfit` / `mzi_fsr_peakfit`），**tol 一律未放宽**，实测残差 rel 1.4e-8% / 2.0e-7% / 2.3e-9%，余量 2000× 以上；反向 10% 扰动三道全 FAIL（B20 最灵敏，≤0.1% 即可检出）。③**护栏升级为三分类双向复核**：非自证桩 ⇒ |diff| 必须非零；自证桩 ⇒ |diff| 必须为零 ⇒ 既能抓「假独立」也能抓「漏分类」。④**路径②三模式全通**：默认 `verified=7` / `--perturb 0.10` 10 / `--ai` 2（诚实值，未误伤）。⚠️ **诚实边界**：自证桩 40/48，光子侧新接三道验证的是「峰位周期性」而非器件全物理。
- ✅ **量子侧五道接线（v0.9.17 · 2026-09-02 · 严格独立 7 → 12）**：B12/B13/B22/B23/B24 五道接成真独立候选（离散 TL 三对角本征 / 441 维电荷基 / 双基对拍 / 三模 Fock 截断），自证桩 40→35；tol 仅 B13 收紧 50×（0.10→2.0e-3）/ B24 按实测重定（1e-6→3e-5）、余未动；判据窗口铁律确立（baseline 残差 < tol < min 反向扰动信号，B13 逐键信号谱披露盲区）；全量回归抓出 B24 候选返回 numpy 标量致 run_harness JSON 序列化 TypeError，已修（candidate 返回值 float 包裹）。⚠️ 诚实边界：独立 12/48、自证桩 35。
- ✅ **S13 设计良率锚接线（v0.9.18 · 2026-09-02 · 严格独立 12 → 13）**：S13（环形 FSR 设计良率 DFY）golden=蒙特卡洛仿真良率（种子 1313/20000 点）↔ candidate=解析高斯积分闭式（Φ 精确解，保留 1/L 非线性）双算法互证；自证桩 35→34；tol 未动（0.01，残差 3.4e-4 余量 29.7×）；🔴 方法学铁律：S7/S8 解析均值是硬编码常量且对 σ 不敏感（均值定理）→ 伪独立陷阱实测证否不接；B15 判「tmm.py 物理模型错配」留 v0.9.19。⚠️ 诚实边界：独立 13/48、自证桩 34。
- ✅ **B15 波导光栅严格求解器接线（v0.9.19 · 2026-09-02 · 严格独立 13 → 14）**：v0.9.18 判「不接」的前提（只有 tmm.py 可用）被推翻——**写正确的求解器本身就是解法**。新写 `lda_solver/bragg_solver.py`：E(z)=n_eff²·(1+m·cos(2πz/Λ)) 的**反周期 Bloch 广义本征值问题** A ψ=β²B ψ（反周期边界锁定 k=±π/Λ，谱最低简并对=第一 Bragg 带隙，中心→λ_B=2π/β_c）↔ golden 一阶相位匹配闭式 λ_B=2·n_eff·Λ，**物理同源、方法独立**（运动学 vs 动力学全波本征谱）；自证桩 34→33；tol 未动（0.01，残差 8.36e-6 余量 1196×；反向 n_eff×1.1 信号 1.55e-1 余 15.5×）；网格 N=240 双向标定（N=480 偶然抵消、N=960 越 LAPACK 地板反升，均避开）；无调制极限自校（简并劈裂 3.7e-13~2.7e-11 机器精度命中 π/(Λ·n_eff)）。⚠️ 诚实边界：独立 14/48、自证桩 33。
- ✅ **B14 定向耦合器接线 + golden 语义修正（v0.9.20 · 2026-09-02 · 严格独立 14 → 15）**：🔴 **D-66「怀疑 golden 本身」第 4 例**——原式 λ/(2Δn)=15.5µm 是**完全转移长度**（P₂=sin²(π/2)=1.0，RK4 数值实证）被错标 3dB 点；真 3dB 点=λ/(4Δn)=**7.75µm**（P₂=sin²(π/4)=0.5）。golden 修正 + 同源消费点 4 处一并修（`_dc_supermode_core` 相位校验 Δβ·L=π→π/2 等）；tol 0.5→0.25 同比重定；kernel_seal smoke 动态跟随 5/5。候选=新写 `lda_solver/dc_cmt_solver.py`：增量 2×2 复传播矩阵数值传播 → P₂(z) FFT 拍频谱峰（Hann 窗+抛物线细化）→ L_3dB=L_P/4（B3/B4/B20 同款「数值序列提取频域周期」方法学，与 golden 闭式反解独立）。🔴 方法学发现：**二模恒耦合系统传播矩阵是精确旋转**，「数值传播+根查找」路线（RK4/分段 TM/采样插值）全部退化为机器精度（8e-15~4.6e-13）→ 撞 1e-12 自证桩判据 ⇒ 必须 FFT 谱峰路线（baseline 1.56e-4 非机器精度）；反向 n_e×1.1 信号 6.44（25.8×）；自证桩 33→32。⚠️ 诚实边界：独立 15/48、自证桩 32。
- ✅ **B1 米氏散射接线（v0.9.21 · 2026-09-02 · 严格独立 15 → 16）**：新写 `lda_solver/mie_solver.py` 完整 Mie 级数（B&H 4.53 + Wiscombe 截断，纯 numpy）↔ golden Rayleigh 一阶极限；|diff|=Rayleigh 固有截断误差（-0.001%@x=0.01 → 1.388%@x=0.4，「x≪1 精确一致」的定量边界）；数值自检双锚（x→0 收敛 + scipy 交叉验证 ≤3e-8）。🔴 **环境确定性修复**：golden `use_miepython=True` 会在装有 miepython 的环境切完整 Mie ⇒ golden 环境相关、判决不可复现——default_params 钉死 False。baseline 3.945e-5（tol=2e-4 未动，余量 5.1×）；反向 m×1.1 信号 2.357e-3（11.9×）；自证桩 32→31。⚠️ 诚实边界：独立 16/48、自证桩 31。
