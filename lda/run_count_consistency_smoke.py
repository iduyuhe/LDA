...
断言维度（全部死标量，LLM 不进判决路径）：
  ...
  3. 题库：BENCHMARK_ORDER 71 题（B1-B51 共 48 题 + E1-E10 10 题 + S1-S13 13 题）
...
class CountConsistencySmoke(unittest.TestCase):
    ...
    def test_benchmark_b30_e9_s13_split(self):
        ...
        # → B 题 31→33；v0.9.79 路径 B 扩基新增 B34/B36/B37/B40/B41（末位跳 B41）→ 38；B-2 再加 B42-B51 → 48。
        self.assertEqual(len(b_ids), 48, f"B 题应 48（B1-B51，缺口 B35/B38/B39 预留），实际 {len(b_ids)}")
        ...
        self.assertEqual(max(b_ids, key=lambda x: int(x[1:])),
                         "B51", f"B 题最大编号应 B51，实际 {max(b_ids, key=lambda x: int(x[1:]))}")
    ...
    def test_readme_benchmark_counts(self):
        ...
        self.assertIn("B1-B51", self.readme)
