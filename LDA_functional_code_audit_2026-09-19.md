...
> ✅ **2026-09-21 更新：第 2 / 3 / 5 / 6 项已由「波次 4 —— 补盲」全部补齐**（报告 `LDA_blindspot_supplement_2026-09-21.md` · 提交 `9563745` · 零代码零发版）。
> 逐项标「已补盲」并附结论与**诚实边界**；第 1 项早在 v0.9.111 已补（见 §5.2）；第 4 项（三方镜像 `vendor/devsim_mirror`）**仍不覆盖**（属外部代码）。
...
2. **未做数值正确性复核**——469 锚的 golden/candidate 数值未逐一复算（属 `lda-anchor-wiring` 技能范畴，历批已做判据 D + 反向测试）。
   ✅ **已补盲（2026-09-21 · T4.4 · 抽查 8 锚，非全量 469）**：报告 §3 —— 结论 **登记值无一是错的**；查出 3 项**方法约定依赖**：① `FSR=λ²/(nL)` 属**一阶微分近似**（与相邻根精确差相差 **≈1/m**；**B20** 该差 `1.04e-3` vs `tol=1e-6` ⇒ **不可能被独立候选升格**）· ② **B21** 算术平均 `n_eff` 与偏振严格零阶值 差 **TE +8.3% / TM −23.5%** · ③ **B3/B4** 的 1/m 差属预期。**B23 获独立佐证**（LC 本征谱恰等间距 `2.000000000`）。⚠️ **边界**：抽样非全量，余下 461 锚未逐一复算。
3. **未审计 WebUI 前端 JS 质量**（`index.html` 368 KB / `store.html` 75 KB 内联脚本未做 lint）。
   ✅ **已补盲（2026-09-21 · T4.2）**：抽取 **8 段内联脚本共 5034 行** ⇒ eslint 9.39.4（隔离安装 · 不进主仓库依赖）**0 error / 0 fatal**；质量信号 `no-unused-vars` **84** · `no-empty` **3**；**判据有效性已验证**（注入 `no-undef` / `no-redeclare` / 语法错 / `no-cond-assign` 四类真缺陷**全部被报出**）。报告 §5。⚠️ **边界**：仅静态 lint，未做运行时行为审计。 ✅ **84 处已闭环（2026-09-21 · v0.9.119）**：逐项甄别 ⇒ **81/84 = 96.4% 为误报**（模板串 `onclick` / HTML 属性 / 跨片段 / 外链 JS）· 3 处零引用中 **1 真死码（`store.html` `stClass`）已删**、2 处属有意保留 ⇒ eslint **84→83** · WebUI 三护栏全绿；**不建棘轮**（eslint 不在仓库 ⇒ 非 hermetic + 误报率过高）。
4. **未审计 `vendor/devsim_mirror` 镜像内容**（三方代码，Apache-2.0，属外部）。
5. **未评估性能/扩展性**（仅有 `bench_router_scale.py` 等基准脚本存在，未实跑）。
   ✅ **已补盲（2026-09-21 · T4.3）**：`bench_router_scale.py` **首次实跑** —— 旧 router 在 **N=40 出现 50× 阶跃**（旧 slope≈**2.63** / 新≈**1.12**）；新路径全 N **连通**（N=320 连通 315）；内置护栏 `320/160 = 2.58× < 4` ⇒ **PASS**。报告 §2。⚠️ **边界**：本机单机单实例基线，非集群/生产压测。
6. **未做依赖漏洞扫描**（无 `pip-audit`/`safety` 环境）。
   ✅ **已补盲（2026-09-21 · T4.1）**：隔离 venv 装 **`pip-audit 2.10.1`**（CI venv 零污染）⇒ 两组模式均 **`No known vulnerabilities found`**。报告 §4。⚠️ **边界**：**optional deps（numba/torch/viz/data/dev）未扫** —— 本轮只扫必装依赖；另 `pip-audit` 自建临时 venv 会继承用户级 `pip.ini` 的清华源（本环境不可用）⇒ 须注入 `PIP_INDEX_URL` 才跑通（已入 `IRONLAWS.md` §六）。
