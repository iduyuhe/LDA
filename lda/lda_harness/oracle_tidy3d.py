"""B6 · 光栅耦合器 3D 场级 ORACLE（Tidy3D，LGPL-2.1 客户端 + 商业云求解，仅外部）。

设计纪律（《白皮书》§11 许可证红线）：Tidy3D 的 Python 客户端为 **LGPL-2.1**（弱
copyleft），求解服务为 Flexcompute 商业云（需 API key + FlexCredit）；**绝不
import 进 LDA Apache-2.0 核心**。本模块是"外部 ORACLE 适配层"——仅在用户显式配置了
...
这是"主权优先"策略下对 B 级依赖（MIT/BSD 可 fork，LGPL/商业云仅外部 ORACLE）的
标准处理：核心永不污染，真值按需外挂。
...
def resolve_tidy3d_grating(params):
    ...
    if not key:
        return None
    try:
        import tidy3d as td  # LGPL-2.1 客户端，仅外部 ORACLE；缺失即不可用
    ...
    #          "note": "Tidy3D 3D FDTD ORACLE（外部，LGPL 不进核心）"}
