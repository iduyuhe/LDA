...
BENCHMARK_DEFS = {
    ...
    "E2": {
        ...
        "note": "实证锚（A 级可溯源）：golden=**实测**群折射率 n_g=1.892 —— "
                ...
                "根因=**网格过粗**：0.3µm 芯厚在 dl=λ/24=64.6nm 下仅约 4.6 格，"
                "阶梯边界随窗口尺寸改变对齐位置 → 离散化误差跳变。"
                "对照：厚 SiN 800×800（约 12 格分辨）n_g 散射仅 0.0015，完全收敛。"
                "（σ 硬编码 2.3 的 latent bug 已修，但实测对现有用例无变化，非主因。）"
                "**故本锚当前只能判定『量级一致 + 判决路径真实』，不能宣称『精度验证』**——"
                "所有测试窗口下均落在容差内（最大 |diff|=0.078 < 0.10），判定本身鲁棒"
                "（已钉进 smoke 窗口鲁棒性断言），但 0.10 容差中约 ±0.08 是数值不确定度、非物理裕度。"
                "根治需**亚网格 ε 平均**（sub-cell averaging）+ 更细网格，登记为 R16。"
