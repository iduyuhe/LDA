...
benchmarks 过滤）无 smoke 覆盖。本 smoke 以库方式走同一 KernelGateway 全链路：
  ...
  1) reference 候选 → 全部 PASS（B1-B41 含 Batch B-1 物理定律 + E1-E7 实证锚 + S1-S13 系统锚，D-104 注入实证锚后；
...
def main() -> int:
    ...
    # 1) reference → 全部 PASS（物理定律 + 实证锚双 ground；B1-B41 含 Batch B-1 + E1-E* + S1-S13）
    ...
    check("list_benchmarks 全题库（动态计数 = BENCHMARK_ORDER 长度）",
          ...
          f"{len(ids)} 题（B1-B41 含 Batch B-1 + E1-E{_n_e} + S1-S13）")
