...
- ⚠️ **v0.9.112 注（设计意图与现实脱节）**：上条所称「小的、固定、快速」的前提是
  子集成员都快；但 `run_d_criterion_smoke.py` 因 ③ 基线残差普查随候选数（448 道）
  增长已到 **~174s** ⇒ 子集实为 ~176s，本文件 ~290s，并**继承该成员的超时预算**
  （2026-09-19 二轮全量：d_criterion TIMEOUT ⇒ 本例 2/3 FAIL，非本文件缺陷）。
- ✅ **v0.9.119（T6.2）已复评并换员**：把 `run_d_criterion_smoke.py` 换成
  `run_verify_waveguide_2d_smoke.py`（~7.7s · 真数值验证）⇒ 子集恢复「小而快」，
  预算继承风险消除。**全量覆盖一分未减**：d_criterion 仍由门禁直接跑。
  本文件预算保持 900s（**只放宽不收紧**：换员后实测大幅下降，900s 属保守留白）。

运行：C:/Users/Administrator/.workbuddy/binaries/python/envs/default/Scripts/python.exe run_ci_industrial_smoke.py
（代表子集 ~10s（v0.9.119 换员后）+ 性能基准 greens ~35s + 负例 ~?s，总量 10 线程
实测见 v0.9.119 全量报告。本项预算 900s ⇒ 换员后余量充裕。）
"""
import os
import sys
import tempfile
...
_SUBSET_CONTRACT = [
    ...
    # ✅ T6.2（2026-09-21 · 波次 6）：原成员 `run_d_criterion_smoke.py` **已换出**。
    #   理由（复评 ③ 项 → 选 ① 换成员）：
    #     · 它含「全部已接线严格独立候选的基线残差普查」（448 道），实测 **~174s**，
    #       且**随候选数线性增长**（不是固定成本）⇒ 直接违背本子集「小的、固定、
    #       快速、**负载无关**」的设计意图（那句设计意图正是 v0.9.28 为消除
    #       「门禁负载诱发抖动」而立的：在已被前序 smoke 压载的机器上再跑一段重载，
    #       会让本文件连带 TIMEOUT 假红）。
    #     · 它还使本文件**继承该成员的超时预算风险**（2026-09-19 二轮全量：
    #       d_criterion TIMEOUT ⇒ 本例 2/3 FAIL，非本文件缺陷）。
    #   替换为 `run_verify_waveguide_2d_smoke.py`（实测 ~7.7s · 真数值验证：
    #   2D FDFD 波导 —— 仍是「验证 harness 类」真求解，不是记账类，故聚合契约的
    #   代表性不降），恢复「小而快」并去掉预算继承风险。
    #   🔴 覆盖未减：`run_d_criterion_smoke` 仍由门禁 `run_ci_regression --tag core`
    #      直接跑（它本就是 CORE_SMOKES 成员），本文件只是**不再嵌套重跑**它。
    "run_verify_waveguide_2d_smoke.py",  # 2D FDFD 验证，~7.7s（v0.9.119 换入）
    "run_b28_nullfit_smoke.py",        # 光子求解器，~3s（实测 0.66s）
]
