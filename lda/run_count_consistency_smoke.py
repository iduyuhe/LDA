...
断言维度（全部死标量，LLM 不进判决路径）：
  ...
  3. 题库：BENCHMARK_ORDER 469 题（B1-B451 共 446 题 + E1-E10 10 题 + S1-S13 13 题）
  4. CI 门禁：CORE_SMOKES 条数（动态）↔ README 顶行 `CI core N 条` 严格一致
  5. README 宣传串：动态构造「22 引擎 + 11 包 = 33 类端到端（光子 15 + 量子 7）」
     「469 题（B1-B451 + E1-E10 + S1-S13）」；反向断言 README 不含已废弃错误串
...
class CountConsistencySmoke(unittest.TestCase):
    ...
    def test_benchmark_b30_e9_s13_split(self):
        ...
        # v0.9.99 B-19 加 B313-B328 → 323；v0.9.100 B-20 加 B329-B344 → 339；v0.9.104 B-22 加 B361-B373 → 373；v0.9.105 B-23 加 B374-B386 → 381；v0.9.106 B-24 加 B387-B399 → 394；v0.9.107 B-25 加 B400-B412 → 407；v0.9.108 B-26 加 B413-B425 → 420；v0.9.109 B-27 加 B426-B438 → 433；v0.9.110 B-28 加 B439-B451 → 446。
        self.assertEqual(len(b_ids), 446, f"B 题应 446（B1-B451，缺口 B35/B38/B39/B69/B72 预留），实际 {len(b_ids)}")
        ...
        self.assertEqual(max(b_ids, key=lambda x: int(x[1:])),
                         "B451", f"B 题最大编号应 B451，实际 {max(b_ids, key=lambda x: int(x[1:]))}")
