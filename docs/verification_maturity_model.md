...
>
> ⚠️ **版本注记（2026-09-16）**：本文档为 **v0.9.62 模型定义稿**，下文 §1/§3/§4/§6 出现的计数（52 锚 / 26 严格独立 / 1 降级 / 25 自证桩）为**当时快照**。**当前账本以 harness 动态推导为准**（`README.md`「当前账本」段 / `GET /api/verification_ledger` / `P1-1_self_certified_discipline.md`）——截至 **v0.9.82**：**84 锚 / 严格独立 63 / 降级 3（E9,E10,B21）/ 自证桩 18（独立率 75.0%）**。另注：`design_rule_anchor` 的三个代表锚 **B5/B6/B7 已全部升 `strict_independent`**（v0.9.81 / v0.9.82），该 provenance 桶当前 **0 道**；下文以三者为例的说明仅作**历史方法学追溯**。
