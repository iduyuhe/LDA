...
- 生成时间：2026-09-02T21:05:33
- L0_IR：(内置默认 B1–B4,B8)
- candidate：IndependentCandidateRouter(独立候选 14 道: B12,B13,B15,B20,B22,B23,B24,B25,B26,B27,B3,B4,B9,S13；降级量级参考 1 道: E2)
- oracle：确定性物理定律锚（analytical/EIM/Airy/Rayleigh）
- self_consistent：True

> ⚠️ **本报告不构成验证结论**：本次运行中 **14 项**由**独立候选求解器**判出（计入 `summary.verified`）；其余项中 **33 项**走 ReferenceCandidate 占位自证（候选值即黄金值、「误差」列恒为 0、恒 PASS，**零验证价值**），**1 项**为降级量级参考（有独立候选但与 golden 几何不同源/精度不足，**不进死标量判决**）。把「N/N 通过」整体读作「N 项已验证」是误读：真正被验证的只有那 14 项。

## 汇总：48/48 通过（独立候选 14 项中 **14 项通过=已验证** · 33 项自证闭环 · 1 项降级量级参考（不进判决），**非验证结论**）
