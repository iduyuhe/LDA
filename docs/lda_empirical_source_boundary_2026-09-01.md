...
4. 🔴 **D-67（2026-09-01）：两个量不是"改判"关系，而是"分离"关系**——本次改判
   `E-YBRANCH-LOSS` 为过量损耗时，v0.9.10 一度把引擎默认输出 `value` 也改成了
   过量损耗，**导致 5 条整芯片链路每个分束器少算 3.0103 dB**
   （GC-PLC-1X8 9.33→0.33、GC-PLC-1X16 12.44→0.44、GC-SENSE 13.65→7.63、
   GC-QKD-TX 13.56→7.54、GC-CPO-8CH 10.63→7.62），且因插损 metric 方向为
   `le`（越小越 PASS）**全部仍显示 PASS = 假绿**，CI core 84/84 全绿也没抓到。
   **教训**：3.01 dB 虽"非器件品质指标"，但它**真实存在且必须计入链路预算**；
   「实证锚对照用哪个量」与「链路预算用哪个量」是**两件事**，只能分离不能替换。
   现引擎同时输出 `split_loss_dB`（= 3.0103 + 过量，链路预算）与
   `excess_loss_dB`（器件品质，实证锚对照），并加装能量守恒下界与 metric
   语义错配两道护栏（详见 `CHANGELOG.md` v0.9.11）。
